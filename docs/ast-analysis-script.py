...
class ASTAnalyzer:
    ...
    def __init__(self):
        ...
        self.performance_critical_ops: List[str] = [
            'read_text', 'write_text', 'read', 'write',  # File I/O operations
            'compile', 'search', 'match',  # Regex operations
            'encode', 'decode',  # Encoding operations
            'split', 'join', 'strip',  # String operations
            'len', 'sum', 'max', 'min'  # Collection operations
        ]
        self.library_usage_patterns: Dict[str, int] = Counter()
        
    def analyze_module_ast(self, file_path: Union[str, Path]) -> Dict[str, Any]:
        """
            ...
            if isinstance(node.func, ast.Name):
                ...
                if isinstance(node.func.value, ast.Name):
                    return {
                        ...
                        'args_count': len(node.args),
                        'type': 'method_call',
                        'is_performance_critical': node.func.attr in self.performance_critical_ops
                    }
        except Exception:
            pass
        return None
    ...
    def analyze_library_usage(self, file_path: Union[str, Path]) -> Dict[str, Any]:
        """
                    ...
                    if module.startswith('flext_core'):
                        for alias in node.names:
                            analysis['flext_core_usage'].append({
                                ...
                                'alias': alias.asname,
                                'line': node.lineno,
                                'impact_level': self._assess_flext_core_impact(alias.name)
                            })
                        ...
                        for alias in node.names:
                            analysis['pydantic_usage'].append({
                                ...
                                'alias': alias.asname,
                                'line': node.lineno,
                                'impact_level': self._assess_pydantic_impact(alias.name)
                            })
                    elif module in ['os', 're', 'pathlib', 'datetime', 'typing', 'collections']:
                        for alias in node.names:
                            analysis['standard_lib_usage'].append({
                                ...
                                'alias': alias.asname,
                                'line': node.lineno,
                                'impact_level': self._assess_standard_lib_impact(alias.name)
                            })
    ...
    def _assess_flext_core_impact(self, component_name: str) -> str:
        ...
        if component_name in high_impact:
            return 'high'
        elif component_name in medium_impact:
            return 'medium'
        else:
            return 'low'
    ...
    def _analyze_call_performance(self, node: ast.Call) -> Optional[Dict[str, Any]]:
        """
        ...
        if isinstance(node.func, ast.Attribute):
            if isinstance(node.func.value, ast.Name):
                ...
                elif method_name in ['compile', 'search', 'match']:
                    return {
                        f'{obj_name}.{method_name}': {
                            'line': node.lineno,
                            'type': 'regex',
                ...
                elif method_name in ['encode', 'decode']:
                    return {
                        f'{obj_name}.{method_name}': {
                            'line': node.lineno,
                            'type': 'encoding',
    ...
    def analyze_call_graph(self, file_path: Union[str, Path]) -> Dict[str, Any]:
        """
    ...
    def _extract_call_graph_info(self, node: ast.Call) -> Optional[Dict[str, Any]]:
        """
            ...
            if isinstance(node.func, ast.Attribute):
                ...
                if isinstance(node.func.value, ast.Name):
                    ...
                    return {
                        ...
                        'line': node.lineno,
                        'args_count': len(node.args),
                        'is_internal': obj_name.startswith('self') or obj_name.startswith('_')
                    }
    ...
    def _is_method_chain(self, node: ast.Call) -> bool:
        ...
        if isinstance(node.func, ast.Attribute):
            if hasattr(node.func.value, 'value'):
                return isinstance(node.func.value.value, ast.Call)
        return False
    
    def generate_comprehensive_report(self, source_directory: Union[str, Path]) -> Dict[str, Any]:
        """
        ...
        Example:
            ...
            >>> print(f"Total complexity: {report['aggregate_statistics']['total_complexity']}")
        """
        ...
        for file_path in python_files:
            ...
            if 'error' not in module_analysis:
                report['aggregate_statistics']['total_complexity'] += module_analysis['complexity']
                report['aggregate_statistics']['total_lines_of_code'] += module_analysis['lines_of_code']
                report['aggregate_statistics']['total_ast_nodes'] += module_analysis['ast_nodes']
                report['aggregate_statistics']['total_function_calls'] += len(module_analysis['calls'])
                report['aggregate_statistics']['total_classes'] += len(module_analysis['classes'])
                report['aggregate_statistics']['total_functions'] += len(module_analysis['functions'])
            
            # Update library usage summary
            if 'error' not in library_analysis:
                for usage in library_analysis['flext_core_usage']:
                    report['library_usage_summary']['flext_core_usage'][f"{usage['module']}.{usage['name']}"] += 1
                
                for usage in library_analysis['pydantic_usage']:
                    report['library_usage_summary']['pydantic_usage'][f"{usage['module']}.{usage['name']}"] += 1
                
                for usage in library_analysis['standard_lib_usage']:
                    report['library_usage_summary']['standard_lib_usage'][f"{usage['module']}.{usage['name']}"] += 1
            ...
            if 'error' not in library_analysis:
                for call, details in library_analysis['performance_impact'].items():
                    ...
                    if impact_level == 'high':
                        report['performance_summary']['high_impact_operations'][call] += 1
                    elif impact_level == 'medium':
                        report['performance_summary']['medium_impact_operations'][call] += 1
                    else:
                        report['performance_summary']['low_impact_operations'][call] += 1
            
            # Update call graph summary
            if 'error' not in call_graph_analysis:
                for call in call_graph_analysis['internal_calls']:
                    report['call_graph_summary']['internal_calls'][call['function']] += 1
                
                for call in call_graph_analysis['external_calls']:
                    report['call_graph_summary']['external_calls'][call['function']] += 1
    ...
    def _generate_recommendations(self, report: Dict[str, Any]) -> List[str]:
        ...
        if total_complexity > 1000:
            recommendations.append("Consider refactoring modules with high complexity scores")
        
        # Analyze library usage patterns
        flext_core_usage = report['library_usage_summary']['flext_core_usage']
        if len(flext_core_usage) < 5:
            recommendations.append("Consider increasing flext-core integration for better consistency")
        
        # Analyze performance characteristics
        high_impact_ops = report['performance_summary']['high_impact_operations']
        if len(high_impact_ops) > 10:
            recommendations.append("Review high-impact operations for optimization opportunities")
        ...
        if len(external_calls) > len(internal_calls) * 2:
            recommendations.append("Consider reducing external dependencies for better maintainability")
...
def main():
    ...
    """
    print("=== FLEXT-LDIF AST Analysis Script ===")
    print("Performing comprehensive AST analysis...")
    ...
    print(f"Total complexity score: {report['aggregate_statistics']['total_complexity']}")
    print(f"Total lines of code: {report['aggregate_statistics']['total_lines_of_code']}")
    print(f"Total AST nodes: {report['aggregate_statistics']['total_ast_nodes']}")
    print(f"Total function calls: {report['aggregate_statistics']['total_function_calls']}")
    
    # Print library usage summary
    print(f"\n=== Library Usage Summary ===")
    print("Flext-Core Usage:")
    for component, count in report['library_usage_summary']['flext_core_usage'].most_common(5):
        print(f"  {component}: {count} usages")
    
    print("Pydantic Usage:")
    for component, count in report['library_usage_summary']['pydantic_usage'].most_common(5):
        print(f"  {component}: {count} usages")
    
    # Print performance summary
    print(f"\n=== Performance Summary ===")
    print("High Impact Operations:")
    for operation, count in report['performance_summary']['high_impact_operations'].most_common(5):
    ...
    print(f"\nAnalysis complete! Detailed results available in module_analyses.")
