...
from flext_ldif import TLdif, FlextLdifEntry, FlextLdifConfig
...
"""
    ...
    def test_parse_valid_ldif_success(self, sample_ldif_content: str) -> None:
        ...
        # Verify first entry
        entry = result.data[0]
        assert str(entry.dn) == "cn=John Doe,ou=people,dc=example,dc=com"
    ...
    def test_round_trip_parsing_writing(self, sample_ldif_content: str) -> None:
        ...
        for original, reparsed in zip(original_entries, reparsed_entries):
    ...
    def test_write_file_to_nonexistent_directory_fails(self, sample_ldif_content: str) -> None:
        ...
        assert "failed" in write_result.error.lower()
...
"""
        ...
        assert parse_time < 5.0  # Should parse 100 entries in under 5 seconds
...
"""
        ...
        entry = result.data[0]
        assert "Üser" in entry.get_attribute("cn")[0]
...
"""
        ...
        entry = result.data[0]
        assert len(entry.get_attribute("description")[0]) == 1000
