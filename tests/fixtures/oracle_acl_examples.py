...
"""
    ...
    BINDMODE_SSL = """dn: cn=admin,dc=example,dc=com
...
orclaci: access to entry by group=cn=admins,dc=example,dc=com (all) DenyGroupOverride
objectClass: person
cn: protected
sn: Protected
"""
...
orclaci: access to entry by group=cn=admins,dc=example,dc=com (add) constraintonaddedobject=(objectclass=person)
...
orclaci: access to entry by group=cn=admins,dc=example,dc=com (browse,read,write) bindmode=(SSLOneway) DenyGroupOverride AppendToAll bindipfilter=(orclipaddress=10.0.0.*) constraintonaddedobject=(objectclass=inetOrgPerson)
objectClass: person
cn: secure
sn: Secure
"""
...
orclaci: access to attr=(userPassword) by group=cn=admins,dc=example,dc=com (all) DenyGroupOverride
objectClass: person
cn: multi
sn: Multiple
"""
...
aci: (targetattr="cn")(targattrfilters="add=cn:(cn=admin)")(version 3.0; acl "Filter CN additions"; allow (add) userdn="ldap:///uid=admin,dc=example,dc=com";)
objectClass: person
cn: testuser
sn: User
"""
    ...
    TARGETCONTROL = """dn: cn=admin,dc=example,dc=com
aci: (targetattr="*")(targetcontrol="1.3.6.1.4.1.42.2.27.9.5.2")(version 3.0; acl "Proxy Auth Control"; allow (read,search) userdn="ldap:///uid=admin,dc=example,dc=com";)
objectClass: person
cn: admin
sn: Administrator
"""
    ...
    EXTOP = """dn: cn=pwdadmin,dc=example,dc=com
aci: (targetattr="*")(extop="1.3.6.1.4.1.26027.1.6.1")(version 3.0; acl "Password Modify ExtOp"; allow (read) userdn="ldap:///uid=pwdadmin,dc=example,dc=com";)
objectClass: person
cn: pwdadmin
...
aci: (targetattr="cn,sn")(targattrfilters="add=cn:(cn=admin)")(targetcontrol="1.3.6.1.4.1.42.2.27.9.5.2")(extop="1.3.6.1.4.1.26027.1.6.1")(version 3.0; acl "Combined target extensions"; allow (add,read) userdn="ldap:///uid=admin,dc=example,dc=com";)
objectClass: person
cn: controlled
sn: Controlled
"""
...
aci: (targetattr="cn,sn,mail")(targattrfilters="add=cn:(cn=admin)")(targetcontrol="1.3.6.1.4.1.42.2.27.9.5.2")(extop="1.3.6.1.4.1.26027.1.6.1")(version 3.0; acl "Maximum security ACL"; allow (add,read,search) userdn="ldap:///uid=admin,dc=example,dc=com" and ip="10.0.0.0/8" and dns="*.secure.example.com" and dayofweek="Mon,Tue,Wed,Thu,Fri" and timeofday >= "0800" and timeofday <= "1800" and authmethod = "ssl" and ssf >= "256";)
objectClass: person
cn: maximum
sn: Maximum
...
aci: (targetattr="*")(version 3.0; acl "Admin full access"; allow (all) userdn="ldap:///uid=admin,dc=example,dc=com" and ip="10.0.0.1";)
objectClass: person
cn: multi
sn: Multiple
"""
...
aci: (targetattr="*")(targattrfilters="add=cn:(cn=admin)")(extop="1.3.6.1.4.1.26027.1.6.1")(version 3.0; acl "OUD specific"; allow (add,read) userdn="ldap:///uid=admin,dc=example,dc=com";)
objectClass: person
cn: oud-only
sn: OUD Only
"""
...
orclaci: access to entry by dn=uid=admin,dc=example,dc=com (browse,read,add)
objectClass: person
cn: oud-only
sn: OUD Only
"""
