...
class DirectoryEntryDataDict(TypedDict):
    """TypedDict for directory entry test data."""
...
class TestDnCaseRegistry(s):
    ...
    registry_with_dns: ClassVar[m.Ldif.DnRegistry]  # pytest fixture
    registry_with_references: ClassVar[m.Ldif.DnRegistry]  # pytest fixture
    ...
    @pytest.fixture
    def registry_with_dns(self) -> m.Ldif.DnRegistry:
        """Create registry with some c.DNs."""
        reg = m.Ldif.DnRegistry()
        reg.register_dn("cn=admin,dc=example,dc=com")
        reg.register_dn("ou=users,dc=example,dc=com")
        return reg

    @pytest.fixture
    def registry_with_references(self) -> m.Ldif.DnRegistry:
    ...
    def test_registry_starts_empty(self, registry: m.Ldif.DnRegistry) -> None:
    ...
    def test_registry_has_empty_internal_structures(
        ...
        assert isinstance(registry._registry, m.DynamicMetadata)
    ...
    def test_normalize_idempotent(self, registry: m.Ldif.DnRegistry) -> None:
    ...
    def test_register_multiple_different_dns(
        ...
        """Test registering multiple different c.DNs."""
        dn1 = "cn=user1,dc=com"
        dn2 = "cn=user2,dc=com"
        dn3 = "ou=users,dc=com"
    ...
    def test_get_canonical_exact_match(
        self,
        registry_with_dns: m.Ldif.DnRegistry,
    ) -> None:
    ...
    def test_has_dn_empty_registry(self, registry: m.Ldif.DnRegistry) -> None:
    ...
    def test_normalize_single_dn_field(
        self,
        registry_with_references: m.Ldif.DnRegistry,
    ) -> None:
    ...
    def test_normalize_mixed_registered_unregistered(
        ...
        """Test normalizing mix of registered and unregistered c.DNs."""
        data: DirectoryEntryDataDict = {
            "dn": "cn=group,dc=com",
            "member": [
                "CN=User1,DC=Com",
    ...
    def test_clear_removes_all_dns(
        ...
        """Test that clear removes all registered c.DNs."""
        registry.register_dn("cn=admin,dc=com")
