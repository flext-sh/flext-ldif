...
"""
        ...
        detection = result.unwrap()  # ServerDetectionResult model
        confidence = detection.confidence
        assert detection.detected_server_type == "oid" or (
...
"""
        ...
        confidence = detection.confidence
        assert detection.is_confident or (
            isinstance(confidence, (int, float)) and confidence >= 0
        )
...
"""
        ...
        confidence = detection.confidence
        assert isinstance(confidence, (int, float)) and confidence >= 0
...
class TestServerDetectorErrorHandling:
    ...
    def test_detect_without_input(self, detector: FlextLdifServerDetector) -> None:
        ...
        assert result.error is not None and "must be provided" in result.error
...
class TestServerDetectorExecute:
    ...
    def test_execute_returns_status(self, detector: FlextLdifServerDetector) -> None:
        ...
        assert result.is_success
        status = result.unwrap()  # ClientStatus model
        assert status.status == "initialized"
        assert hasattr(status, "services")
        # Verify services list contains detector capabilities
