...
class TestFlextLdifValidatorService:
    ...
    def test_execute_invalid_config(self):
        ...
        mock_config.validate_business_rules.return_value = FlextResult[None].fail("Invalid config")
    ...
    def test_validate_entry_business_rules_failure(self):
        ...
        mock_entry.validate_business_rules.return_value = FlextResult[None].fail("Business rule failed")
    ...
    def test_validate_configuration_rules_no_config(self):
    ...
    def test_validate_configuration_rules_non_strict(self):
    ...
    def test_validate_configuration_rules_strict_valid(self):
    ...
    def test_validate_configuration_rules_empty_attribute_list(self):
        ...
        assert result.is_failure
        assert FlextLdifValidationMessages.EMPTY_ATTRIBUTES_NOT_ALLOWED.format(attr_name="cn") in result.error

    def test_validate_configuration_rules_empty_string_value(self):
        ...
        assert result.is_failure
        assert FlextLdifValidationMessages.EMPTY_ATTRIBUTE_VALUE_NOT_ALLOWED.format(attr_name="cn") in result.error

    def test_validate_configuration_rules_whitespace_only_value(self):
        ...
        mock_entry = Mock(spec=FlextLdifEntry)
        mock_attributes = Mock()
        mock_attributes.attributes = {"cn": ["   ", "valid"]}  # Contains whitespace-only
        mock_entry.attributes = mock_attributes
    ...
    def test_validate_ldif_entries(self):
    ...
    def test_validate_entries_single_valid_entry(self):
    ...
    def test_validate_entries_multiple_valid_entries(self):
    ...
    def test_validate_entries_with_failure(self):
        ...
        mock_entry = Mock(spec=FlextLdifEntry)
        mock_entry.validate_business_rules.return_value = FlextResult[None].fail("Invalid entry")
        ...
        assert FlextLdifValidationMessages.ENTRY_VALIDATION_FAILED.lower() in result.error
        assert "Invalid entry" in result.error

    def test_validate_dn_format_success(self):
        """Test validate_dn_format with valid DN."""
        service = FlextLdifValidatorService()
        
        with patch('flext_ldif.validator_service.LdifValidator.validate_dn') as mock_validate:
    ...
    def test_validate_dn_format_failure(self):
    ...
    def test_validate_entries_first_entry_fails(self):
        """Test validate_entries when first entry fails."""
        service = FlextLdifValidatorService()
        
        # Create mock failing entry at index 0
        mock_entry = Mock(spec=FlextLdifEntry)
        mock_entry.validate_business_rules.return_value = FlextResult[None].fail("First entry invalid")
    ...
    def test_configuration_rules_allow_empty_attributes_true(self):
