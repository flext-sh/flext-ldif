...
class TestServerDetectorInitialization:
    ...
    def test_execute_returns_status(self) -> None:
        ...
        # ClientStatus is a Pydantic model, not a dict
...
objectClass: person
orclaci: (targetentry="cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com")(version 3.0;acl "REDACTED_LDAP_BIND_PASSWORD";allow(all)
"""
        result = detector.detect_server_type(ldif_content=ldif_content)
        assert result.is_success
...
"""
        ...
        assert isinstance(detection.patterns_found, list)
        assert any("Oracle OID" in p for p in detection.patterns_found)
...
"""
        ...
        assert hasattr(detection, "patterns_found")
        assert isinstance(detection.patterns_found, list)
        assert any("OpenLDAP" in p for p in detection.patterns_found)
...
"""
    ...
    def test_detect_active_directory_by_attributes(self) -> None:
        ...
        ldif_content = """version: 1
dn: cn=REDACTED_LDAP_BIND_PASSWORD,cn=users,dc=example,dc=com
objectClass: person
samAccountName: REDACTED_LDAP_BIND_PASSWORD
...
"""
        ...
        # ServerDetectionResult is a Pydantic model, not a dict
        assert hasattr(detection, "scores")
        assert isinstance(detection.scores, dict)
        # Should have scores for all server types
        assert "oid" in detection.scores
        assert "oud" in detection.scores
        assert "openldap" in detection.scores
        assert "active_directory" in detection.scores
...
"""
        ...
        detection = result.unwrap()
        assert isinstance(detection.patterns_found, list)
        assert all(isinstance(p, str) for p in detection.patterns_found)
