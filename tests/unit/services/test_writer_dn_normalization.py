...
class TestsFlextLdifsFlextLdifWriterDnNormalization(s):
    ...
    def test_parse_simple_dn_into_components(self, dn_service: FlextLdifDn) -> None:
        ...
        tm.that(len(components) == 4, eq=True), "DN should have 4 components"
        attr_names = [comp[0] for comp in components]
        tm.that("cn" in attr_names, eq=True), "Should contain cn component"
        tm.that("ou" in attr_names, eq=True), "Should contain ou component"
        tm.that("dc" in attr_names, eq=True), "Should contain dc component"
    ...
    def test_parse_dn_with_escaped_characters(self, dn_service: FlextLdifDn) -> None:
        ...
        result = dn_service.parse_components(dn)
        tm.that(result.is_success, eq=True), "Escaped DN parsing should succeed"
        components = result.value
        tm.that(len(components) >= 3, eq=True), "DN should have at least 3 components"
        attr_names = [comp[0] for comp in components]
        tm.that("cn" in attr_names, eq=True), "Should contain cn attribute"
        tm.that("ou" in attr_names, eq=True), "Should contain ou attribute"
        tm.that("dc" in attr_names, eq=True), "Should contain dc attribute"
    ...
    def test_normalize_dn_to_lowercase(self, dn_service: FlextLdifDn) -> None:
        ...
        tm.that(result.is_success, eq=True), "DN normalization should succeed"
        normalized = result.value
        (
            tm.that(normalized.startswith("cn="), eq=True),
            "Attribute names should be lowercase",
        )
        tm.that("ou=" in normalized.lower(), eq=True), "Should contain ou component"
        tm.that("dc=" in normalized.lower(), eq=True), "Should contain dc component"
    ...
    def test_validate_dn_format_valid(self, dn_service: FlextLdifDn) -> None:
        ...
        tm.that(is_valid is True, eq=True), "Valid DN should pass format validation"
    ...
    def test_validate_dn_format_invalid(self, dn_service: FlextLdifDn) -> None:
        ...
        result = dn_service.validate_format(dn)
        tm.that(result.is_success, eq=True), "Validation check should complete"
        is_valid = result.value
        tm.that(is_valid is False, eq=True), "Malformed DN should fail validation"
    ...
    def test_clean_dn_removes_spacing_issues(self, dn_service: FlextLdifDn) -> None:
        ...
        tm.that("cn=" in cleaned, eq=True), "Should remove spaces around ="
        tm.that(len(cleaned) < len(dn), eq=True), "Cleaned DN should be shorter"
    ...
    def test_escape_dn_value_special_characters(self, dn_service: FlextLdifDn) -> None:
        ...
        tm.that(escaped != value, eq=True), "Should escape special characters"
        (
    ...
    def test_unescape_dn_value_hex_escapes(self, dn_service: FlextLdifDn) -> None:
        ...
        tm.that("," in unescaped, eq=True), "Hex-escaped comma should be unescaped"
