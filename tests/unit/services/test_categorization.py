...
class TestsFlextLdifCategorization(s):
    ...
    class TestDNValidation:
        ...
        def test_validate_dns_valid_entries(self) -> None:
            ...
            entries = [
                TestsFlextLdifCategorization.Factories.create_user_entry(),
                TestsFlextLdifCategorization.Factories.create_group_entry(),
            ]
            service = FlextLdifCategorization()
            result = service.validate_dns(entries)
            validated = tm.ok(result)
            assert len(validated) == 2

        def test_validate_dns_invalid_dn_rejected(self) -> None:
            """Test validate_dns() rejects invalid c.DNs."""
            # Use m.Ldif.Entry directly to create entry with invalid DN
            invalid_entry = m.Ldif.Entry(
                dn="invalid dn format",
                attributes={c.Names.CN: [c.Values.TEST]},
            )
            service = FlextLdifCategorization()
    ...
    class TestSchemaDetection:
        ...
        def test_is_schema_entry_with_attributetypes(self) -> None:
            ...
            entry = TestsFlextLdifCategorization.Factories.create_schema_entry()
            service = FlextLdifCategorization()
        ...
        def test_is_schema_entry_with_objectclasses(self) -> None:
            ...
            entry = s().create_entry(
                TestsFlextLdifCategorization.Constants.DN_SCHEMA,
                {
                    c.Names.OBJECTCLASS: [c.Names.TOP],
        ...
        def test_is_schema_entry_non_schema(self) -> None:
            ...
            entry = TestsFlextLdifCategorization.Factories.create_user_entry()
            service = FlextLdifCategorization()
    ...
    class TestEntryCategorization:
        ...
        def test_categorize_entry_by_type(
            ...
            factory_method = getattr(TestsFlextLdifCategorization.Factories, entry_factory)
        ...
        def test_categorize_entry_rejected_no_match(self) -> None:
            ...
            assert category == TestsFlextLdifCategorization.Constants.CATEGORY_REJECTED
        ...
        def test_categorize_entry_with_server_type_oid(self) -> None:
            ...
            service = FlextLdifCategorization(
                server_type=TestsFlextLdifCategorization.Constants.SERVER_OID,
            )
            category, _reason = service.categorize_entry(entry)
            assert category in {
                TestsFlextLdifCategorization.Constants.CATEGORY_USERS,
                TestsFlextLdifCategorization.Constants.CATEGORY_REJECTED,
            }
        ...
        def test_categorize_entry_hierarchy_priority(self) -> None:
            ...
            assert category == TestsFlextLdifCategorization.Constants.CATEGORY_HIERARCHY
    ...
    class TestEntriesCategorization:
        ...
        def test_categorize_entries_mixed_types(self) -> None:
            ...
            entries = [
                TestsFlextLdifCategorization.Factories.create_user_entry(),
                TestsFlextLdifCategorization.Factories.create_group_entry(),
                TestsFlextLdifCategorization.Factories.create_hierarchy_entry(),
                TestsFlextLdifCategorization.Factories.create_schema_entry(),
            ]
            service = FlextLdifCategorization()
            result = service.categorize_entries(entries)
            categories = tm.ok(result)
            assert len(categories[TestsFlextLdifCategorization.Constants.CATEGORY_USERS]) == 1
            assert len(categories[TestsFlextLdifCategorization.Constants.CATEGORY_GROUPS]) == 1
            assert len(categories[TestsFlextLdifCategorization.Constants.CATEGORY_HIERARCHY]) == 1
            assert len(categories[TestsFlextLdifCategorization.Constants.CATEGORY_SCHEMA]) == 1
        ...
        def test_categorize_entries_tracks_metadata(self) -> None:
            ...
            categories = tm.ok(result)
            categorized_entry = categories[TestsFlextLdifCategorization.Constants.CATEGORY_USERS][
                0
            ]
        ...
        def test_categorize_entries_tracks_rejected_metadata(self) -> None:
            ...
            result = service.categorize_entries([entry_with_stats])
            categories = tm.ok(result)
            assert len(categories[TestsFlextLdifCategorization.Constants.CATEGORY_REJECTED]) > 0
            rejected = categories[TestsFlextLdifCategorization.Constants.CATEGORY_REJECTED][0]
            assert rejected.metadata is not None
            assert rejected.metadata.processing_stats is not None
            assert rejected.metadata.processing_stats.was_rejected
            assert (
    ...
    class TestBaseDNFiltering:
        ...
        def test_filter_by_base_dn_includes_matching(self) -> None:
            ...
            entries = [
                TestsFlextLdifCategorization.Factories.create_user_entry(
                    f"cn=user1,{TestsFlextLdifCategorization.Constants.DN_BASE}",
                ),
                TestsFlextLdifCategorization.Factories.create_user_entry(
                    f"cn=user2,{TestsFlextLdifCategorization.Constants.DN_BASE}",
                ),
            ]
            service = FlextLdifCategorization(
                base_dn=TestsFlextLdifCategorization.Constants.DN_BASE,
            )
            # FlexibleCategories is a type alias, use the actual class
            categories = _FlexibleCategories()
            categories[TestsFlextLdifCategorization.Constants.CATEGORY_USERS] = entries
            filtered = service.filter_by_base_dn(categories)
            assert len(filtered[TestsFlextLdifCategorization.Constants.CATEGORY_USERS]) == 2
        ...
        def test_filter_by_base_dn_excludes_outside_base(self) -> None:
            ...
            categories[TestsFlextLdifCategorization.Constants.CATEGORY_USERS] = entries
            filtered = service.filter_by_base_dn(categories)
            assert len(filtered[TestsFlextLdifCategorization.Constants.CATEGORY_USERS]) == 1
        ...
        def test_filter_by_base_dn_tracks_metadata(self) -> None:
            ...
            # FlexibleCategories is a type alias, use the actual class
            categories = _FlexibleCategories()
            categories[TestsFlextLdifCategorization.Constants.CATEGORY_USERS] = [entry_with_stats]
            filtered = service.filter_by_base_dn(categories)
            filtered_entry = filtered[TestsFlextLdifCategorization.Constants.CATEGORY_USERS][0]
        ...
        def test_filter_by_base_dn_skips_schema_rejected(self) -> None:
            ...
            schema_entry = TestsFlextLdifCategorization.Factories.create_schema_entry()
            rejected_entry = s().create_entry(
            ...
            # FlexibleCategories is a type alias, use the actual class
            categories = _FlexibleCategories()
            categories[TestsFlextLdifCategorization.Constants.CATEGORY_SCHEMA] = [schema_entry]
            categories[TestsFlextLdifCategorization.Constants.CATEGORY_REJECTED] = [
                rejected_entry,
            ]
            filtered = service.filter_by_base_dn(categories)
            assert len(filtered[TestsFlextLdifCategorization.Constants.CATEGORY_SCHEMA]) == 1
            assert len(filtered[TestsFlextLdifCategorization.Constants.CATEGORY_REJECTED]) == 1

        def test_filter_by_base_dn_no_base_dn_configured(self) -> None:
            """Test filter_by_base_dn() returns unchanged if no base DN."""
            entries = [TestsFlextLdifCategorization.Factories.create_user_entry()]
            service = FlextLdifCategorization()
            # FlexibleCategories is a type alias, use the actual class
            categories = _FlexibleCategories()
            categories[TestsFlextLdifCategorization.Constants.CATEGORY_USERS] = entries
    ...
    class TestStaticMethods:
        ...
        def test_filter_categories_by_base_dn(self) -> None:
            ...
            filtered = FlextLdifCategorization.filter_categories_by_base_dn(
                categories,
                TestsFlextLdifCategorization.Constants.DN_BASE,
            )
            assert len(filtered[TestsFlextLdifCategorization.Constants.CATEGORY_USERS]) == 1
            assert len(filtered[TestsFlextLdifCategorization.Constants.CATEGORY_REJECTED]) == 1
    ...
    class TestDependencyInjection:
        ...
        def test_uses_injected_server_registry(self) -> None:
            ...
            service = FlextLdifCategorization(
                server_type=TestsFlextLdifCategorization.Constants.SERVER_RFC,
    ...
    class TestProperties:
        ...
        def test_schema_whitelist_rules_property(self) -> None:
            ...
            rules = m.Ldif.WhitelistRules(allowed_attribute_oids=[OIDs.CN])
