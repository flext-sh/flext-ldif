...
@pytest.fixture
def dn_service() -> FlextLdifDnService:
...
class TestDnValidation:
    ...
    def test_validate_simple_dn(self, dn_service: FlextLdifDnService) -> None:
        """Validate simple DN."""
        result = dn_service.validate_format("cn=John,dc=example,dc=com")
        assert result.is_success
        assert result.unwrap() is True

    def test_validate_complex_dn(self, dn_service: FlextLdifDnService) -> None:
    ...
    def test_validate_dn_missing_value(self, dn_service: FlextLdifDnService) -> None:
...
class TestDnNormalization:
    ...
    def test_normalize_with_spaces(self, dn_service: FlextLdifDnService) -> None:
...
class TestDnCleaning:
    ...
    def test_clean_spaces_around_equals(self, dn_service: FlextLdifDnService) -> None:
    ...
    def test_clean_already_clean_dn(self, dn_service: FlextLdifDnService) -> None:
        """Clean already clean DN (no changes)."""
        original = "cn=John,dc=example,dc=com"
        cleaned = dn_service.clean_dn(original)
        assert cleaned == original

    def test_clean_multiple_spaces(self, dn_service: FlextLdifDnService) -> None:
...
class TestDnEscaping:
    ...
    def test_escape_comma(self, dn_service: FlextLdifDnService) -> None:
        """Escape comma in DN value."""
        escaped = dn_service.escape_dn_value("Smith, John")
        assert escaped == "Smith\\2c John"

    def test_escape_hash(self, dn_service: FlextLdifDnService) -> None:
    ...
    def test_escape_leading_space(self, dn_service: FlextLdifDnService) -> None:
        """Escape leading space in DN value."""
        escaped = dn_service.escape_dn_value(" admin")
        assert escaped.startswith("\\")

    def test_escape_trailing_space(self, dn_service: FlextLdifDnService) -> None:
        """Escape trailing space in DN value."""
        escaped = dn_service.escape_dn_value("admin ")
        # Trailing space gets hex escaped to \20
        assert escaped.endswith("\\20")

    def test_escape_empty_value(self, dn_service: FlextLdifDnService) -> None:
        """Escape empty DN value."""
        escaped = dn_service.escape_dn_value("")
        assert escaped == ""

    def test_unescape_hex_format(self, dn_service: FlextLdifDnService) -> None:
        """Unescape hex format DN value."""
        unescaped = dn_service.unescape_dn_value("Smith\\2c John")
        assert unescaped == "Smith, John"

    def test_unescape_backslash_escape(self, dn_service: FlextLdifDnService) -> None:
        """Unescape backslash escaped DN value."""
        unescaped = dn_service.unescape_dn_value("Smith\\, John")
        assert unescaped == "Smith, John"

    def test_unescape_no_escapes(self, dn_service: FlextLdifDnService) -> None:
        """Unescape DN value with no escapes."""
        unescaped = dn_service.unescape_dn_value("John Smith")
        assert unescaped == "John Smith"

    def test_unescape_empty_value(self, dn_service: FlextLdifDnService) -> None:
        """Unescape empty DN value."""
        unescaped = dn_service.unescape_dn_value("")
        assert unescaped == ""

    def test_roundtrip_escape_unescape(self, dn_service: FlextLdifDnService) -> None:
...
class TestDnComparison:
    ...
    def test_compare_invalid_dn(self, dn_service: FlextLdifDnService) -> None:
        ...
        result = dn_service.compare_dns("cn=admin,dc=example,dc=com", "invalid dn")
        assert result.is_failure
...
class TestRdnParsing:
    ...
    def test_parse_simple_rdn(self, dn_service: FlextLdifDnService) -> None:
        """Parse simple RDN."""
    ...
    def test_parse_multivalued_rdn(self, dn_service: FlextLdifDnService) -> None:
    ...
    def test_parse_rdn_with_escaped_plus(self, dn_service: FlextLdifDnService) -> None:
        """Parse RDN with escaped plus character."""
        result = dn_service.parse_rdn(r"cn=Test\+User")
        assert result.is_success

    def test_parse_empty_rdn(self, dn_service: FlextLdifDnService) -> None:
        """Parse empty RDN should fail."""
        result = dn_service.parse_rdn("")
        assert result.is_failure

    def test_parse_invalid_rdn(self, dn_service: FlextLdifDnService) -> None:
...
class TestDnServiceBuilderPattern:
    ...
    def test_builder_normalize(self, dn_service: FlextLdifDnService) -> None:
    ...
    def test_builder_clean(self, dn_service: FlextLdifDnService) -> None:
...
class TestDnServiceExecutePattern:
    ...
    def test_execute_compare(self, dn_service: FlextLdifDnService) -> None:
...
class TestCaseRegistry:
    ...
    def test_register_dn(self, dn_service: FlextLdifDnService) -> None:
    ...
    def test_get_canonical_dn(self, dn_service: FlextLdifDnService) -> None:
    ...
    def test_has_dn(self, dn_service: FlextLdifDnService) -> None:
        """Check if DN is registered."""
        registry = FlextLdifDnService.CaseRegistry()
        registry.register_dn("CN=Admin,DC=Example,DC=Com")
        assert registry.has_dn("cn=admin,dc=example,dc=com") is True
        assert registry.has_dn("cn=unknown,dc=example,dc=com") is False

    def test_get_case_variants(self, dn_service: FlextLdifDnService) -> None:
    ...
    def test_registry_clear(self, dn_service: FlextLdifDnService) -> None:
        """Clear registry."""
        registry = FlextLdifDnService.CaseRegistry()
        registry.register_dn("cn=admin,dc=example,dc=com")
        assert registry.has_dn("cn=admin,dc=example,dc=com") is True
        registry.clear()
        assert registry.has_dn("cn=admin,dc=example,dc=com") is False

    def test_registry_stats(self, dn_service: FlextLdifDnService) -> None:
...
class TestDnServiceIntegration:
    ...
    def test_workflow_clean_and_escape(self, dn_service: FlextLdifDnService) -> None:
