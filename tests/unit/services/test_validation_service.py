...
class TestsTestFlextLdifValidation(s):
    ...
    class Factories:
        ...
        def parametrize_dn_components(cls) -> list[tuple[str, str, bool]]:
            ...
            return [
                (c.Names.CN, "John Smith", True),
                ("CN", "test", True),
                (c.Names.CN, "", True),  # Empty values allowed
                ("2invalid", "test", False),  # Invalid attribute
                (c.Names.CN, "Smith, John", True),  # Special chars allowed
            ]
    ...
    class TestBuilderPattern:
        ...
        def test_with_attribute_names(self) -> None:
            ...
            # Builder pattern returns new instance (immutable model)
            assert result is not builder
            assert result.attribute_names == ["cn", "mail"]
        ...
        def test_with_objectclass_names(self) -> None:
            ...
            # Builder pattern returns new instance (immutable model)
            assert result is not builder
            assert result.objectclass_names == ["person"]
        ...
        def test_with_max_attr_value_length(self) -> None:
            ...
            assert (
                builder.max_attr_value_length != 1024
