...
class TestDnServiceNormalize:
    ...
    def test_normalize_invalid_dn(self, dn_service: FlextLdifDnService) -> None:
        ...
        assert not result.is_success
        assert result.error is not None and "Failed to normalize" in result.error
...
class TestDnServiceNormalizeDnValue:
    ...
    def dn_map(self) -> dict[str, str]:
        ...
        return {
            ...
            "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com": "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com",
        }
...
class TestDnServiceNormalizeDnReferencesInEntry:
    ...
    def test_normalize_dn_references_manager_attribute(
        ...
        normalized = result.unwrap()
        attributes = cast(
            "dict[str, object]", normalized[FlextLdifConstants.DictKeys.ATTRIBUTES]
        )
        assert attributes["manager"] == "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com"
    ...
    def test_normalize_dn_references_list_values(
        ...
        entry: dict[str, object] = {
            FlextLdifConstants.DictKeys.DN: "cn=test,dc=example,dc=com",
            FlextLdifConstants.DictKeys.ATTRIBUTES: {
                "member": ["cn=user1,dc=example,dc=com", "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com"],
                "cn": ["group1"],
            },
        }
        ref_attrs = {"member"}
        ...
        assert result.is_success
        normalized = result.unwrap()
        members = cast("list[str]", normalized[FlextLdifConstants.DictKeys.ATTRIBUTES])
    ...
    def test_normalize_dn_references_non_string_values(
        ...
        entry: dict[str, object] = {
            FlextLdifConstants.DictKeys.DN: "cn=test,dc=example,dc=com",
            FlextLdifConstants.DictKeys.ATTRIBUTES: {
                "manager": ["cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com", 12345],
                "cn": ["user1"],
            },
        }
        ref_attrs = {"manager"}
        ...
        assert result.is_success
        normalized = result.unwrap()
        attributes = normalized[FlextLdifConstants.DictKeys.ATTRIBUTES]
    ...
    def test_normalize_dn_references_non_dict_attributes(
        ...
        assert result.is_success
        normalized = result.unwrap()
        # Check that attributes field exists (empty dict is valid)
        assert FlextLdifConstants.DictKeys.ATTRIBUTES in normalized
        assert isinstance(normalized[FlextLdifConstants.DictKeys.ATTRIBUTES], dict)
...
class TestDnServiceNormalizeAciDnReferences:
    ...
    def dn_map(self) -> dict[str, str]:
        ...
        return {
            "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com": "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com",
            "ou=REDACTED_LDAP_BIND_PASSWORDs,dc=example,dc=com": "ou=REDACTED_LDAP_BIND_PASSWORDs,dc=example,dc=com",
        }
    ...
    def test_normalize_aci_ldap_uri(
        ...
        result = dn_service.normalize_aci_dn_references(
            cast("dict[str, object]", entry), dn_map
        )
        assert result.is_success
        normalized = result.unwrap()
        aci = normalized[FlextLdifConstants.DictKeys.ATTRIBUTES]
    ...
    def test_normalize_aci_quoted_dn(
        ...
        entry = {
            FlextLdifConstants.DictKeys.DN: "cn=test,dc=example,dc=com",
            FlextLdifConstants.DictKeys.ATTRIBUTES: {
                "aci": ['grant (read) groupdn="ou=REDACTED_LDAP_BIND_PASSWORDs,dc=example,dc=com";'],
            },
        }
        result = dn_service.normalize_aci_dn_references(
            cast("dict[str, object]", entry), dn_map
    ...
    def test_normalize_aci_non_dict_attributes(
        ...
        assert result.is_success
        normalized = result.unwrap()
        assert normalized[FlextLdifConstants.DictKeys.ATTRIBUTES]
...
class TestDnServiceExecute:
    ...
    def test_execute_returns_success(self, dn_service: FlextLdifDnService) -> None:
        ...
        assert "RFC 4514" in cast("str", status["rfc_compliance"])
