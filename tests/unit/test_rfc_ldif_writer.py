...
class TestRfcLdifWriterBasic:
    ...
    def test_write_simple_entry_to_file(
        ...
        output_file: Path = temp_output_dir / "simple.ldif"
    ...
    def test_write_entry_to_string(
        ...
        assert result.is_success
        output_data: FlextLdifTypes.Models.CustomDataDict = result.unwrap()
        content = output_data.get("content", "")
        assert isinstance(content, str) and "version: 1" in content
        assert isinstance(content, str) and "dn: cn=test,dc=example,dc=com" in content
...
class TestRfcLdifWriterAppendMode:
    ...
    def test_append_mode_preserves_existing(
        ...
        writer1 = FlextLdifRfcLdifWriter(params=params1, quirk_registry=quirk_registry)
        ...
        writer2 = FlextLdifRfcLdifWriter(params=params2, quirk_registry=quirk_registry)
...
class TestRfcLdifWriterInvalidTypes:
    ...
    def test_invalid_output_file_type(
        ...
        params: dict[str, object] = {
            FlextLdifConstants.DictKeys.OUTPUT_FILE: "/tmp/test.ldif",
            FlextLdifConstants.DictKeys.ENTRIES: [
                {
                    FlextLdifConstants.DictKeys.DN: "cn=test,dc=example,dc=com",
                    FlextLdifConstants.DictKeys.ATTRIBUTES: {
    ...
    def test_invalid_entries_type(
        ...
        params = {
            FlextLdifConstants.DictKeys.ENTRIES: "invalid_string_not_list",
        }
