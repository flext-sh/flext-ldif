...
class TestDnPatternFiltering:
    ...
    def test_multiple_dn_patterns(
        ...
        builder = EntryFilterBuilder()
        builder.with_dn_pattern("*,ou=users,dc=example,dc=com")
        builder.with_dn_pattern("*,ou=REDACTED_LDAP_BIND_PASSWORDs,dc=example,dc=com")
...
class TestCombinedFilters:
    ...
    def sample_entries(self) -> list[FlextLdifModels.Entry]:
        ...
        return [
            ...
            create_test_entry(
                "cn=REDACTED_LDAP_BIND_PASSWORD1,ou=REDACTED_LDAP_BIND_PASSWORDs,dc=example,dc=com",
                {"cn": ["REDACTED_LDAP_BIND_PASSWORD1"], "objectclass": ["person"]},
            ),
            create_test_entry(
...
class TestEdgeCases:
    ...
    def test_with_dn_patterns_helper_method(self) -> None:
        ...
        patterns = [
            "*,ou=users,dc=example,dc=com",
            "*,ou=REDACTED_LDAP_BIND_PASSWORDs,dc=example,dc=com",
        ]
...
class TestFluentApiChaining:
    ...
    def test_chain_with_multiple_patterns(self) -> None:
        ...
        result = (
            EntryFilterBuilder()
            .with_dn_pattern("*,ou=users,dc=example,dc=com")
            .with_dn_pattern("*,ou=REDACTED_LDAP_BIND_PASSWORDs,dc=example,dc=com")
            .apply(entries)
        )
...
class TestIntegration:
    ...
    def test_realistic_user_filtering_scenario(self) -> None:
        ...
        entries = [
            ...
            create_test_entry(
                "cn=REDACTED_LDAP_BIND_PASSWORD.root,ou=REDACTED_LDAP_BIND_PASSWORDs,dc=example,dc=com",
                {
                    "cn": ["REDACTED_LDAP_BIND_PASSWORD.root"],
    ...
    def test_group_membership_filtering(self) -> None:
        ...
        entries = [
            ...
            create_test_entry(
                "cn=REDACTED_LDAP_BIND_PASSWORDistrators,ou=groups,dc=example,dc=com",
                {"cn": ["REDACTED_LDAP_BIND_PASSWORDistrators"], "objectclass": ["groupOfNames"]},
            ),
            create_test_entry(
