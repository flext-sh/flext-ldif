...
class TestFlextLdifAclsCoordinator:
    ...
    def test_builder_build_admin_permission(self) -> None:
        """Test Builder.build_admin_permission method."""
        coordinator = FlextLdifAcls()

        result = coordinator.builder.build_admin_permission(
            target_dn="dc=example,dc=com", subject_dn="cn=admin,dc=example,dc=com"
        )
    ...
    def test_converter_to_ad(self) -> None:
        ...
        acl = FlextLdifModels.UnifiedAcl.create(
            name="test_acl",
            target=FlextLdifModels.AclTarget(dn="dc=example,dc=com"),
            subject=FlextLdifModels.AclSubject(dn="cn=admin"),
            permissions=FlextLdifModels.AclPermissions(read=True, write=True),
            server_type="ad",
            raw_acl="access to dc=example,dc=com by cn=admin read,write",
        ).unwrap()
