...
class TestUtilityFunctions:
    ...
    def test_validate_url_scheme_valid(self):
        ...
        _validate_url_scheme("https://example.com/file.ldif")
        _validate_url_scheme("http://example.com/file.ldif")
    ...
    def test_validate_url_scheme_invalid(self):
        ...
        with pytest.raises(ValueError, match="URL scheme 'ftp' not allowed"):
            _validate_url_scheme("ftp://example.com/file.ldif")
    ...
    def test_safe_url_fetch_success(self):
        ...
        with unittest.mock.patch('flext_ldif.format_handler_service.urllib3.PoolManager') as mock_pool:
            ...
            result = _safe_url_fetch("https://example.com/test.ldif")
            
            assert result == "dn: cn=test,dc=example,dc=com\nobjectClass: person"
            mock_http.request.assert_called_once_with("GET", "https://example.com/test.ldif")
    ...
    def test_safe_url_fetch_http_error(self):
        ...
        with unittest.mock.patch('flext_ldif.format_handler_service.urllib3.PoolManager') as mock_pool:
            ...
            with pytest.raises(ValueError, match="HTTP 404: Failed to fetch"):
                _safe_url_fetch("https://example.com/nonexistent.ldif")
    ...
    def test_safe_url_fetch_network_error(self):
        ...
        with unittest.mock.patch('flext_ldif.format_handler_service.urllib3.PoolManager') as mock_pool:
            ...
            with pytest.raises(ValueError, match="urllib3 fetch error for.*Network unreachable"):
                _safe_url_fetch("https://example.com/test.ldif")

    def test_safe_url_fetch_invalid_url_scheme(self):
        """Test URL fetching with invalid URL scheme."""
        with pytest.raises(ValueError, match="URL scheme 'ftp' not allowed"):
            _safe_url_fetch("ftp://example.com/test.ldif")
    ...
    def test_safe_url_fetch_encoding_error(self):
        ...
        with unittest.mock.patch('flext_ldif.format_handler_service.urllib3.PoolManager') as mock_pool:
            ...
            with pytest.raises(ValueError, match="urllib3 fetch error for"):
                _safe_url_fetch("https://example.com/test.ldif", encoding="utf-8")
    ...
    def test_safe_url_fetch_custom_encoding(self):
        ...
        with unittest.mock.patch('flext_ldif.format_handler_service.urllib3.PoolManager') as mock_pool:
            ...
            result = _safe_url_fetch("https://example.com/test.ldif", encoding="latin-1")
...
class TestFlextLDIFWriter:
    ...
    def test_needs_base64_encoding_unsafe_string(self):
        ...
        assert writer._needs_base64_encoding("description", " starts with space") is True
    ...
    def test_unparse_multiple_entries(self):
        ...
        writer.unparse("dc=example,dc=com", {"objectClass": ["dcObject"], "dc": ["example"]})
        writer.unparse("ou=people,dc=example,dc=com", {"objectClass": ["organizationalUnit"], "ou": ["people"]})
    ...
    def test_unparse_with_line_wrapping_over_boundary(self):
        ...
        desc_lines = [line for line in lines if line.startswith("description:") or line.startswith(" ")]
        ...
        assert len(continuation_lines) > 0, "Expected continuation lines with leading space"
...
class TestFlextLDIFParser:
    ...
    def test_parser_initialization_with_ignored_attrs(self):
        ...
        parser = FlextLDIFParser(
            ldif_content,
            ignored_attr_types=["modifiersName", "modifyTimestamp"]
        )
...
"""
        ...
        assert len(entries) == 1
        dn, record = entries[0]
        assert "very long name that needs to be folded across multiple lines" in dn
        assert "very long name that needs to be folded across multiple lines" in record["cn"][0]
...
"""
        parser = FlextLDIFParser(ldif_content, ignored_attr_types=["modifiersName", "modifyTimestamp"])
        entries = list(parser.parse())
    ...
    def test_parse_simple_entry_only(self):
        """Test parsing LDIF with simple entry."""
        ldif_content = """dn: dc=example,dc=com
objectClass: dcObject
dc: example
...
"""
    ...
    def test_parse_url_reference(self):
        ...
        with unittest.mock.patch('flext_ldif.format_handler_service._safe_url_fetch') as mock_fetch:
...
"""
    ...
    def test_parse_strict_mode_invalid_utf8(self):
        """Test parser in strict mode with invalid UTF-8."""
        # This test simulates invalid UTF-8 in DN processing
        # The actual UTF-8 validation happens in _decode_value method
        import unittest.mock
...
"""
        ...
        # Mock the _decode_value method to simulate UTF-8 error
        with unittest.mock.patch.object(parser, '_decode_value') as mock_decode:
            mock_decode.side_effect = ValueError("Invalid UTF-8 in dn: 'utf-8' codec can't decode")
...
"""
        ...
        # Mock _error method to test strict mode error handling
        with unittest.mock.patch.object(parser, '_error') as mock_error:
            mock_error.side_effect = ValueError("Test error in strict mode")
            
            # Test that _error raises ValueError in strict mode
            with pytest.raises(ValueError, match="Test error in strict mode"):
                parser._error("Test error message")
...
"""
        ...
        with unittest.mock.patch('flext_ldif.format_handler_service.logger') as mock_logger:
            parser._error("Test warning message")
            mock_logger.warning.assert_called_once_with("LDIF parsing warning: %s", "Test warning message")
...
"""
    ...
    def test_parse_version_line_at_start(self):
        """Test parsing LDIF with version line at start of entry."""
        ldif_content = """dn: cn=test,dc=example,dc=com
objectClass: person
version: 1
cn: test
...
"""
        ...
        # Should skip version line within entry and parse normally
        assert len(entries) == 1
        dn, record = entries[0]
        assert dn == "cn=test,dc=example,dc=com"
        assert record["objectClass"] == ["person"]
        assert record["cn"] == ["test"]
        # version should not be in the record
        assert "version" not in record

    def test_parse_empty_lines_handling(self):
        """Test parsing LDIF with empty lines properly handled."""
        ldif_content = """dn: cn=test,dc=example,dc=com
...
"""
        ...
        # Should handle empty lines and parse entry correctly
        assert len(entries) == 1
        dn, record = entries[0]
        assert dn == "cn=test,dc=example,dc=com"
        assert record["objectClass"] == ["person"]
...
"""
    ...
    def test_modernized_ldif_write_with_params(self):
        ...
        entries = [
            ("cn=Test,dc=example,dc=com", {"cn": ["Test"], "description": [" starts with space"]})
        ]
...
"""
    ...
    def test_modernized_ldif_parse_error_handling(self):
        ...
        with unittest.mock.patch('flext_ldif.format_handler_service.FlextLDIFParser') as mock_parser_class:
    ...
    def test_modernized_ldif_write_error_handling(self):
        ...
        with unittest.mock.patch('flext_ldif.format_handler_service.FlextLDIFWriter') as mock_writer_class:
