...
""")
    ...
    def test_categorize_entry_with_multiple_objectclasses(self, tmp_path: Path) -> None:
        ...
        ldif_file = input_dir / "test.ldif"
        ldif_file.write_text("""dn: cn=admin,dc=example,dc=com
objectClass: top
objectClass: person
...
""")
    ...
    def test_dn_with_special_characters(self, tmp_path: Path) -> None:
        ...
        ldif_file = input_dir / "test.ldif"
        ldif_file.write_text(r"""dn: cn=user\, admin,dc=example,dc=com
objectClass: person
cn: user, admin
sn: Admin
""")
...
objectClass: person
aci: (target="") (version 3.0; acl "admin"; allow(all) (userdn="ldap:///cn=admin,dc=example,dc=com");)
cn: admin
"""
        ldif_file.write_text(ldif_content)
