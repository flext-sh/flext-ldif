...
Tests the DnCaseRegistry class that tracks canonical DN case during
...
class TestDnCaseRegistryInitialization:
...
class TestDnNormalization:
    ...
    def test_normalize_removes_spaces(self, registry: DnCaseRegistry) -> None:
    ...
    def test_normalize_converts_to_lowercase(self, registry: DnCaseRegistry) -> None:
        ...
        assert normalized == "cn=admin,dc=example,dc=com"
        assert normalized.islower()

    def test_normalize_preserves_structure(self, registry: DnCaseRegistry) -> None:
    ...
    def test_normalize_handles_multiple_spaces(self, registry: DnCaseRegistry) -> None:
    ...
    def test_normalize_idempotent(self, registry: DnCaseRegistry) -> None:
...
class TestDnRegistration:
    ...
    def test_register_tracks_all_case_variants(self, registry: DnCaseRegistry) -> None:
    ...
    def test_register_multiple_different_dns(self, registry: DnCaseRegistry) -> None:
...
class TestCanonicalDnRetrieval:
    ...
    def test_get_canonical_exact_match(self, registry: DnCaseRegistry) -> None:
    ...
    def test_get_canonical_different_case(self, registry: DnCaseRegistry) -> None:
        """Test getting canonical DN with different case."""
        canonical = registry.get_canonical_dn("CN=Admin,DC=Example,DC=Com")
        assert canonical == "cn=admin,dc=example,dc=com"

    def test_get_canonical_with_spaces(self, registry: DnCaseRegistry) -> None:
...
class TestDnExistenceCheck:
    ...
    def test_has_dn_exact_match(self, registry: DnCaseRegistry) -> None:
        """Test has_dn with exact match."""
        assert registry.has_dn("cn=admin,dc=com")
        assert registry.has_dn("ou=users,dc=com")

    def test_has_dn_different_case(self, registry: DnCaseRegistry) -> None:
        """Test has_dn is case-insensitive."""
        assert registry.has_dn("CN=Admin,DC=Com")
        assert registry.has_dn("cn=ADMIN,dc=COM")
        assert registry.has_dn("OU=Users,DC=Com")

    def test_has_dn_with_spaces(self, registry: DnCaseRegistry) -> None:
        """Test has_dn handles spaces."""
        assert registry.has_dn("cn=admin, dc=com")
        assert registry.has_dn("ou=users, dc=com")

    def test_has_dn_unregistered_returns_false(self, registry: DnCaseRegistry) -> None:
...
class TestCaseVariantsTracking:
    ...
    def test_get_case_variants_single_variant(self, registry: DnCaseRegistry) -> None:
...
class TestOudConsistencyValidation:
    ...
    def test_validate_empty_registry_success(self, registry: DnCaseRegistry) -> None:
    ...
    def test_validate_multiple_inconsistent_dns(self, registry: DnCaseRegistry) -> None:
...
class TestRegistryStatistics:
    ...
    def test_stats_multiple_dns_mixed_variants(self) -> None:
        ...
        assert stats["dns_with_multiple_variants"] == 2  # admin and user1
...
class TestEdgeCases:
    ...
    def test_normalize_dn_references_with_non_list_non_string_value(
        ...
        registry = DnCaseRegistry()
        registry.register_dn("cn=admin,dc=com")
        data: FlextTypes.Dict = {"dn": "cn=admin,dc=com", "someField": 123}
...
__all__ = [
    ...
    "TestDnCaseRegistryInitialization",
