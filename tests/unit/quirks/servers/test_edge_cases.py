...
class TestsFlextLdifEdgeCases(s):
    ...
    def test_unicode_names(self, ldif_api: FlextLdif) -> None:
        ...
        u.Tests.Matchers.that(len(entries) > 0, eq=True)
        for entry in entries:
            u.Tests.Matchers.that(entry.dn is not None, eq=True)
            if entry.dn is not None:
                u.Tests.Matchers.that(entry.dn.value, eq=True)
    ...
    def test_deep_dn(self, ldif_api: FlextLdif) -> None:
        ...
        (
            u.Tests.Matchers.that(max_depth > 5, eq=True),
    ...
    def test_large_multivalue(self, ldif_api: FlextLdif) -> None:
        ...
        (
            u.Tests.Matchers.that(max_values >= 10, eq=True),
    ...
    def test_roundtrip_unicode(self, ldif_api: FlextLdif, tmp_path: Path) -> None:
        ...
        u.Tests.Matchers.that(len(entries) == 1, eq=True)
        ...
        u.Tests.Matchers.that(len(roundtrip_entries) == 1, eq=True)
