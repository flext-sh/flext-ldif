...
class TestFlextLdifExceptionsMissingCoverage:
    ...
    def test_validation_error_with_dn_context(self) -> None:
        ...
        assert "(DN: uid=test,ou=people,dc=example,dc=com)" in result.error
    ...
    def test_validation_error_with_attribute_context(self) -> None:
        ...
        assert result.is_success is False
        assert "Test validation error" in result.error
        assert "(Attribute: objectClass)" in result.error
    ...
    def test_validation_error_with_rule_context(self) -> None:
        ...
        assert result.is_success is False
        assert "Test validation error" in result.error
        assert "(Rule: required_attribute)" in result.error
    ...
    def test_processing_error_with_operation_context(self) -> None:
        ...
        assert "(Operation: parse_ldif)" in result.error
    ...
    def test_processing_error_with_entry_count_context(self) -> None:
        ...
        assert result.is_success is False
        assert "Test processing error" in result.error
        assert "(Entries: 42)" in result.error
    ...
    def test_configuration_error_with_config_key_context(self) -> None:
        ...
        assert result.is_success is False
        assert "Test configuration error" in result.error
        assert "(Config: ldif.parser.max_line_length)" in result.error
    ...
    def test_validation_error_with_non_string_dn(self) -> None:
        ...
        assert result.is_success is False
        assert "Test validation error" in result.error
        assert "(DN:" not in result.error  # Should not add DN context
    ...
    def test_validation_error_with_non_string_attribute(self) -> None:
        ...
        assert result.is_success is False
        assert "Test validation error" in result.error
        assert "(Attribute:" not in result.error  # Should not add attribute context
    ...
    def test_validation_error_with_non_string_rule(self) -> None:
        ...
        assert result.is_success is False
        assert "Test validation error" in result.error
        assert "(Rule:" not in result.error  # Should not add rule context
    ...
    def test_processing_error_with_non_string_operation(self) -> None:
        ...
        assert result.is_success is False
        assert "Test processing error" in result.error
        assert "(Operation:" not in result.error  # Should not add operation context
    ...
    def test_processing_error_with_non_int_entry_count(self) -> None:
        ...
        assert result.is_success is False
        assert "Test processing error" in result.error
        assert "(Entries:" not in result.error  # Should not add entry count context
    ...
    def test_entry_error_with_entry_data_context(self) -> None:
        ...
        assert "(Attributes:" in result.error
        assert (
    ...
    def test_entry_error_with_entry_data_few_attributes(self) -> None:
        ...
        assert "Test entry error" in result.error
        assert "(Attributes:" in result.error
        assert "objectClass, cn" in result.error  # Should show all attributes
    ...
    def test_entry_error_with_non_dict_entry_data(self) -> None:
        ...
        assert result.is_success is False
        assert "Test entry error" in result.error
        assert "(Attributes:" not in result.error  # Should not add attributes context
    ...
    def test_create_method_with_other_error_type(self) -> None:
        ...
        assert result.is_success is False
        assert "Test generic error" in result.error
