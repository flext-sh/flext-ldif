...
class TestFlextLdifAclParser:
    ...
    def test_parse_openldap_acl_complex(self) -> None:
        ...
        parser = FlextLdifAclParser()
        acl_string = "access to dn.subtree=dc=example,dc=com by dn=cn=admin,dc=example,dc=com write"
        result = parser.parse_openldap_acl(acl_string)
        assert isinstance(result, FlextResult)
        if result.is_success:
            acl = result.unwrap()
    ...
    def test_parse_389ds_acl_complex(self) -> None:
        ...
        parser = FlextLdifAclParser()
        acl_string = 'aci: (target="ldap:///dc=example,dc=com")(targetattr="*")(version 3.0; acl "Admin Access"; allow (all) userdn="ldap:///cn=admin,dc=example,dc=com";)'
        result = parser.parse_389ds_acl(acl_string)
        assert isinstance(result, FlextResult)
        if result.is_success:
            acl = result.unwrap()
    ...
    def test_parse_oracle_acl(self) -> None:
        ...
        parser = FlextLdifAclParser()
        acl_string = "orclaci: access to entry by cn=admin,dc=example,dc=com read"
    ...
    def test_parse_oracle_acl_complex(self) -> None:
        ...
        parser = FlextLdifAclParser()
        acl_string = "orclaci: access to dn.subtree=dc=example,dc=com by group cn=admins,dc=example,dc=com write,read"
        result = parser.parse_oracle_acl(acl_string)
        assert isinstance(result, FlextResult)
        if result.is_success:
            acl = result.unwrap()
    ...
    def test_parse_acl_oracle_format(self) -> None:
        ...
        parser = FlextLdifAclParser()
        acl_string = "orclaci: access to entry by cn=admin read"
...
class TestFlextLdifAclParserEdgeCases:
    ...
    def test_parse_openldap_with_special_chars(self) -> None:
        ...
        parser = FlextLdifAclParser()
        acl_string = "access to * by cn=user\\,admin,dc=example,dc=com read"
        result = parser.parse_openldap_acl(acl_string)
        assert isinstance(result, FlextResult)
    ...
    def test_parse_oracle_with_special_chars(self) -> None:
        ...
        parser = FlextLdifAclParser()
        acl_string = (
            "orclaci: access to entry by cn=admin\\,root,dc=example,dc=com read"
        )
        result = parser.parse_oracle_acl(acl_string)
        assert isinstance(result, FlextResult)
    ...
    def test_parse_oracle_very_long_acl(self) -> None:
        ...
        parser = FlextLdifAclParser()
        acl_string = "orclaci: access to " + ("entry" * 100) + " by cn=admin read"
        result = parser.parse_oracle_acl(acl_string)
        assert isinstance(result, FlextResult)
