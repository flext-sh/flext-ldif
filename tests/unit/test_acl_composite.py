...
class TestAclCompositePattern:
    ...
    def test_evaluate_acl_rules_list(self) -> None:
        ...
        rules = [
            ...
            service.create_subject_rule("cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com"),
        ]
...
class TestAclServiceIntegration:
    ...
    def test_complete_acl_workflow(self) -> None:
        ...
        # Create complex rule: (read AND write) OR REDACTED_LDAP_BIND_PASSWORD
        ...
        REDACTED_LDAP_BIND_PASSWORD_rule = service.create_subject_rule("cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com")

        final_rule = service.create_composite_rule(operator="OR")
        final_rule.add_rule(permissions_rule)
        final_rule.add_rule(REDACTED_LDAP_BIND_PASSWORD_rule)

        # Test scenarios
        REDACTED_LDAP_BIND_PASSWORD_context: dict[str, object] = {
            "permissions": {"read": False, "write": False},
            "subject_dn": "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com",
        }
        assert final_rule.evaluate(REDACTED_LDAP_BIND_PASSWORD_context).value is True
