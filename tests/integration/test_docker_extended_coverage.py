...
class TestFilteringAndTransformation:
    ...
    def test_api_filter_with_custom_callback(
        ...
        # Custom filter: entries with cn containing 'REDACTED_LDAP_BIND_PASSWORD' (or just filter all)
        filter_result = api.filter(
            entries,
            custom_filter=lambda e: "REDACTED_LDAP_BIND_PASSWORD" in str(e.dn).lower(),
        )
