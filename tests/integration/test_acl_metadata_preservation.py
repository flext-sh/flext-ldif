...
"""
        ...
        assert (
            entry.metadata.extensions.get(FlextLdifConstants.MetadataKeys.ACL_BINDMODE)
            == "Simple"
        )
        assert (
...
aci: (targetattr="cn")(targattrfilters="add=cn:(cn=admin)")(version 3.0; acl "test"; allow (read) userdn="ldap:///self";)
objectClass: person
cn: test
"""
        ...
        assert targattrfilters == "add=cn:(cn=admin)", "targattrfilters not preserved"
...
"""
        ...
        extop = entry.metadata.extensions.get(FlextLdifConstants.MetadataKeys.ACL_EXTOP)
...
"""
        ...
        ssf = entry.metadata.extensions.get(FlextLdifConstants.MetadataKeys.ACL_SSF)
    ...
    def test_oud_all_features_combined(self, api: FlextLdif) -> None:
        ...
        oud_ldif = """dn: cn=test,dc=example,dc=com
aci: (targetattr="cn")(targattrfilters="add=cn:(cn=admin)")(targetcontrol="1.3.6.1.4.1.42.2.27.9.5.2")(extop="1.3.6.1.4.1.26027.1.6.1")(version 3.0; acl "test"; allow (read) userdn="ldap:///self" and ip="192.168.1.0/24" and dns="*.example.com" and dayofweek="Mon,Tue,Wed" and timeofday >= "0800" and authmethod = "ssl" and ssf >= "128";)
objectClass: person
cn: test
"""
        result = api.parse(oud_ldif, server_type="oud")
        ...
        assert (
            entry.metadata.extensions.get(FlextLdifConstants.MetadataKeys.ACL_EXTOP)
            == "1.3.6.1.4.1.26027.1.6.1"
        )

        # Verify bind rules
        assert (
            entry.metadata.extensions.get(FlextLdifConstants.MetadataKeys.ACL_BIND_IP)
            == "192.168.1.0/24"
        )
        assert (
            entry.metadata.extensions.get(FlextLdifConstants.MetadataKeys.ACL_BIND_DNS)
            == "*.example.com"
        )
        assert (
            entry.metadata.extensions.get(
        ...
        assert (
            entry.metadata.extensions.get(FlextLdifConstants.MetadataKeys.ACL_SSF)
            is not None
        )
...
"""
    ...
    def test_oud_aci_round_trip(self, api: FlextLdif) -> None:
        ...
        original_ldif = """dn: cn=test,dc=example,dc=com
aci: (targetattr="*")(targattrfilters="add=cn:(cn=admin)")(version 3.0; acl "test"; allow (read) userdn="ldap:///self" and ip="192.168.1.0/24";)
objectClass: person
cn: test
"""
        # Parse
