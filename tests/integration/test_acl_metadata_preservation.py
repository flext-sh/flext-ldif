...
class TestOidAclMetadataPreservation:
    ...
    def test_oid_bindmode_preservation(self, api: ldif) -> None:
        ...
        result = api.parse_ldif(oid_ldif, server_type="oid")
...
class TestOudAciMetadataPreservation:
    ...
    def test_oud_targattrfilters_preservation(self, api: ldif) -> None:
        ...
        result = api.parse_ldif(oud_ldif, server_type="oud")
        assert result.is_success, f"Parse failed: {result.error}"
        entries = result.value
        assert len(entries) == 1
        entry = entries[0]
...
class TestAclRoundTripPreservation:
    ...
    def test_oid_acl_round_trip(self, api: ldif) -> None:
        ...
        parse_result = api.parse_ldif(original_ldif, server_type="oid")
        ...
        reparse_result = api.parse_ldif(written_ldif, server_type="oid")
    ...
    def test_oud_aci_round_trip(self, api: ldif) -> None:
        ...
        parse_result = api.parse_ldif(original_ldif, server_type="oud")
        assert parse_result.is_success
        entries = parse_result.value
        entry = entries[0]
        ...
        assert write_result.is_success
        written_ldif = write_result.value
        reparse_result = api.parse_ldif(written_ldif, server_type="oud")
        assert reparse_result.is_success
        reparsed_entries = reparse_result.value
        reparsed_entry = reparsed_entries[0]
        reparsed_extensions = _entry_extensions(reparsed_entry)
