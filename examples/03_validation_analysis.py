...
def parallel_validation_processing() -> FlextResult[FlextLdifModels.ValidationResult]:
...
def advanced_statistical_analysis() -> FlextResult[FlextLdifModels.EntryAnalysisResult]:
...
"""
    ...
    insights = {
        ...
        "objectclass_distribution": stats.objectclass_distribution,
        "patterns_detected": stats.patterns_detected,
        "unique_objectclasses": stats.unique_objectclasses,
    }

    # Calculate advanced metrics
    objectclass_dist = cast("dict[str, int]", insights["objectclass_distribution"])
    if objectclass_dist:
        max_oc = max(objectclass_dist.values())
        insights["most_common_objectclass"] = [
            oc for oc, count in objectclass_dist.items() if count == max_oc
        ]

    attribute_stats = cast(
        "dict[str, dict[str, object]]", insights.get("attribute_statistics")
    )
    if attribute_stats:
        multi_valued = [
            attr
            for attr, stats_data in attribute_stats.items()
            if isinstance(stats_data, dict)
            and cast("float", stats_data.get("multi_valued_ratio", 0)) > 0.1
        ]
        insights["multi_valued_attributes"] = multi_valued

    return FlextResult.ok(stats)


def batch_validation_pipeline() -> FlextResult[dict[str, object]]:
...
def railway_validation_with_analysis() -> FlextResult[dict[str, object]]:
...
def performance_analytics_pipeline() -> FlextResult[dict[str, object]]:
    ...
    return FlextResult.ok(cast("dict[str, object]", performance_results))
