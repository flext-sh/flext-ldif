...
class FlextLdifServersRfcSchema(FlextLdifServersBase.Schema):
    ...
    def should_filter_out_objectclass(
        self,
        _objectclass: m.SchemaObjectClass,
    ) -> bool:
    ...
    def _build_attribute_metadata(
        ...
        server_type_to_use = _server_type or c.ServerTypes.RFC
    ...
    def _parse_attribute(
        ...
        def parse_parts_hook(
            ...
            # Use u.Schema.parse_attribute which returns dict directly
            # Extract only the fields needed by ParsePartsHook
            parsed = u.Schema.parse_attribute(definition)
            # Type narrowing: cast to expected type
            return cast("dict[str, str | bool | None]", parsed)

        # DSL: Use config-based parse signature
        parse_result: FlextResult[m.SchemaAttribute] = u.Parsers.Attribute.parse(
    ...
    def _parse_attribute_core(
        ...
        try:
            ...
            parsed = u.Schema.parse_attribute(attr_definition)
            ...
            if syntax_validation and isinstance(syntax_validation, dict):
                error_value = syntax_validation.get(
                    c.MetadataKeys.SYNTAX_VALIDATION_ERROR,
                )
    ...
    def _parse_objectclass(
        ...
        def parse_parts_hook(
            ...
            # Use u.Schema.parse_objectclass which returns dict directly
            # Extract only the fields needed by ParsePartsHook
            parsed = u.Schema.parse_objectclass(definition)
            # Type narrowing: cast to expected type
            return cast("dict[str, str | list[str] | None]", parsed)

        # DSL: Use config-based parse signature
        # ObjectClass.parse accepts config=None with **kwargs
        parse_result: FlextResult[m.SchemaObjectClass] = u.Parsers.ObjectClass.parse(
    ...
    def _build_objectclass_metadata(
        ...
        metadata = m.QuirkMetadata(
            ...
            extensions=m.DynamicMetadata(**metadata_extensions)
    ...
    def _parse_objectclass_core(
        ...
        """
        try:
            parsed = u.Schema.parse_objectclass(oc_definition)
            ...
            for key, value in metadata_extensions_raw_dict.items():
                if isinstance(value, (str, bool, list)) or value is None:
                    ...
                    metadata_extensions[key] = str(value)
            metadata_extensions[c.MetadataKeys.ORIGINAL_FORMAT] = oc_definition.strip()
    ...
    def _build_attribute_parts(
        ...
        Delegates to u.Schema.build_attribute_parts_with_metadata()
        ...
        """
        return u.Schema.build_attribute_parts_with_metadata(
            attr_data,
    ...
    def _build_objectclass_parts(
        ...
        Delegates to u.Schema.build_objectclass_parts_with_metadata()
        for SRP compliance. Restores original formatting from metadata when
        available for zero data loss (perfect round-trip).
        ...
        """
        return u.Schema.build_objectclass_parts_with_metadata(
            oc_data,
            restore_original=True,
        )
    ...
    def _write_schema_item(
        self,
        data: m.SchemaAttribute | m.SchemaObjectClass,
    ) -> FlextResult[str]:
        ...
        try:
            ...
            if isinstance(data, m.SchemaAttribute):
                ...
                if attr_transformed.metadata:
                    ...
                    if fmt:
                        attr_case = getattr(
                            fmt,
                            "attribute_case",
                            c.SchemaFields.ATTRIBUTE_TYPES,
                        )
                        attr_types_lower = c.SchemaFields.ATTRIBUTE_TYPES.lower()
                ...
                return FlextResult[str].ok(
                    self._ensure_x_origin(
                        transformed_str,
                        cast("m.QuirkMetadata | None", attr_transformed.metadata),
                    ),
                )
            ...
            return FlextResult[str].ok(
                self._ensure_x_origin(
                    transformed_str,
                    cast("m.QuirkMetadata | None", oc_transformed.metadata),
                ),
            )

        except (ValueError, TypeError, AttributeError) as e:
            item_type = (
                "attribute" if isinstance(data, m.SchemaAttribute) else "objectclass"
            )
            logger.exception(
    ...
    def _write_attribute(
        ...
        if not isinstance(attr_data, m.SchemaAttribute):
            return FlextResult[str].fail(
    ...
    def _write_objectclass(
        ...
        if not isinstance(oc_data, m.SchemaObjectClass):
            return FlextResult[str].fail(
    ...
    def __call__(
        ...
        attr_model: m.SchemaAttribute | None = None,
        oc_model: m.SchemaObjectClass | None = None,
    ...
    def __call__(
        ...
        data: str | m.SchemaAttribute | m.SchemaObjectClass | None = None
    ...
    def __new__(
        ...
        **kwargs: t.FlexibleKwargsMutable,
        ...
        if cls.auto_execute:
            ...
            attr_mod: m.SchemaAttribute | None = (
                attr_mod_raw if isinstance(attr_mod_raw, m.SchemaAttribute) else None
            )
            oc_mod_raw = kwargs.get("oc_model")
            oc_mod: m.SchemaObjectClass | None = (
                oc_mod_raw if isinstance(oc_mod_raw, m.SchemaObjectClass) else None
            )
            op_raw = kwargs.get("operation")
            op: c.LiteralTypes.ParseOperationLiteral | None = (
    ...
    def create_metadata(
        self,
        original_format: str,
        extensions: t.MetadataDictMutable | None = None,
    ) -> m.QuirkMetadata:
        ...
        Returns:
            m.QuirkMetadata with quirk_type from Constants of parent server class
        ...
        server_type_value: c.LiteralTypes.ServerTypeLiteral = "generic"
        ...
        return m.QuirkMetadata(
            quirk_type=server_type_value,
            extensions=m.DynamicMetadata(**all_extensions)
            if all_extensions
            else m.DynamicMetadata(),
        )
    ...
    def extract_schemas_from_ldif(
        ...
        *,
        ...
        try:
            ...
            attributes_parsed = u.Schema.extract_attributes_from_lines(
                ldif_content,
            ...
            if validate_dependencies:
                available_attrs = u.Schema.build_available_attributes_set(
                    attributes_parsed,
                )
            ...
            objectclasses_parsed = u.Schema.extract_objectclasses_from_lines(
                ldif_content,
