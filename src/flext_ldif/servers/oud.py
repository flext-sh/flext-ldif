...
from flext_core import FlextLogger, FlextMixins, FlextResult, FlextRuntime
...
# Pydantic models for validation rules structure
class EncodingRules(BaseModel):
    """Encoding rules for server validation."""

    default_encoding: str
    allowed_encodings: list[str]


class DnCaseRules(BaseModel):
    """DN case rules for server validation."""

    preserve_case: bool
    normalize_to: str | None


class AclFormatRules(BaseModel):
    """ACL format rules for server validation."""
...
class ValidationRules(BaseModel):
    """Validation rules for server-specific validation."""
...
class FlextLdifServersOud(FlextLdifServersRfc):
    ...
    def get_categorization_rules(cls) -> FlextLdifModels.CategoryRules:
        ...
        Returns:
            CategoryRules model with OUD server quirks
        ...
        group_ocs = list(
            cls.Constants.CATEGORY_OBJECTCLASSES.get("groups", frozenset()),
        )
    ...
    @staticmethod
    def _build_oud_validation_rules() -> ValidationRules:
        """Build OUD-specific validation rules model.

        Returns:
            ValidationRules model with OUD-specific validation rules
    ...
    def _inject_validation_rules_static(
        ...
        # Use flext-core helper for consistent model serialization
        entry.metadata.extensions["validation_rules"] = (
            FlextMixins.ModelConversion.to_dict(validation_rules)
        )
        ...
        for attr_name, attr_values in attrs_dict.items():
            ...
            for value in values_list:
                ...
                try:
                    ...
                    logger.warning(
                        "Corrected invalid UTF-8 in attribute",
                        attribute_name=attr_name,
                        original_value_preview=(
                            value[: FlextLdifServersOud.Constants.MAX_LOG_LINE_LENGTH]
                            if len(value)
    ...
    def _get_oud_schema_quirk(
        ...
        Returns:
            Schema quirk instance with parse_attribute, write_attribute,
            parse_objectclass, write_objectclass methods

        Note:
            Returns FlextLdifServersBase.Schema (base type) which is the common
            type for both parent_quirk.schema_quirk and new instance.
    ...
    def _process_attribute_type_value(
        ...
        Parse with RFC quirks (OUD inherits from RFC), write with OUD quirks.
        ...
        """
        # Parse as attribute using RFC quirks (OUD inherits from RFC)
        # Get OUD schema quirk which inherits from RFC
        oud_schema_quirk = FlextLdifServersOud._get_oud_schema_quirk(parent_quirk)
        parse_result = oud_schema_quirk.parse_attribute(value_to_parse)
        ...
        write_result = oud_schema_quirk.write_attribute(attr_model)
        ...
        # Clean SYNTAX quotes for OUD compatibility
        # OUD requires SYNTAX OID without quotes (RFC 4512 § 4.1.2)
        written = write_result.unwrap()
        cleaned = FlextLdifServersOud.Schema.clean_syntax_quotes(written)
        cleaned_str = cleaned.decode("utf-8") if isinstance(cleaned, bytes) else cleaned
        return FlextResult[str].ok(cleaned_str)
    ...
    def _process_object_class_value(
        ...
        """
        # Parse as objectClass using RFC quirks (OUD inherits from RFC)
        # Get OUD schema quirk which inherits from RFC
        oud_schema_quirk = FlextLdifServersOud._get_oud_schema_quirk(parent_quirk)
        parse_result = oud_schema_quirk.parse_objectclass(value_to_parse)
        ...
        write_result = oud_schema_quirk.write_objectclass(oc_model)
    ...
    def _process_single_schema_value(
        ...
        if schema_type_lower == "attributetypes":
            ...
            cleaned_value = (
                cleaned.decode("utf-8") if isinstance(cleaned, bytes) else cleaned
            )
    ...
    def _validate_aci_macros_static(aci_value: str) -> FlextResult[bool]:
        ...
        logger.debug(
            ...
            aci_preview=aci_value[: FlextLdifConstants.ACI_PREVIEW_LENGTH]
            if len(aci_value) > FlextLdifConstants.ACI_PREVIEW_LENGTH
            else aci_value,
    ...
    def _resolve_acl_original_names_static(
        ...
        if not transformations or not FlextRuntime.is_dict_like(transformations):
            return acl_attrs
    ...
    def _create_entry_metadata_with_acl_comments_static(
        ...
        if FlextRuntime.is_dict_like(removed_attrs):
    ...
    def _hook_pre_write_entry_static(
        ...
        # All attribute normalizations already done in parsing phase
    ...
    def _comment_acl_attributes_static(
        ...
        acl_attrs, remaining_attrs = (
            FlextLdifServersOud._separate_acl_attributes_static(
        ...
        new_entry_metadata = (
            FlextLdifServersOud._create_entry_metadata_with_acl_comments_static(
    ...
    class Schema(FlextLdifServersRfc.Schema):
        ...
        **Validation**: Verify protocol compliance with:
        ...
        def _add_source_metadata_fields(
            self,
            validated_attr: FlextLdifModels.SchemaAttribute,
            ...
            # Preserve SOURCE values (OUD format)
            if validated_attr.syntax:
                validated_attr.metadata.extensions[
                    meta_keys.SCHEMA_SOURCE_SYNTAX_OID
                ] = str(validated_attr.syntax)
            if validated_attr.name:
                validated_attr.metadata.extensions[
                    meta_keys.SCHEMA_SOURCE_ATTRIBUTE_NAME
                ] = validated_attr.name

        def _add_target_metadata_fields(
            self,
            validated_attr: FlextLdifModels.SchemaAttribute,
            meta_keys: type[FlextLdifConstants.MetadataKeys],
            source_rules: dict[str, str],
        ) -> None:
            """Add target metadata fields (OUD: SOURCE == TARGET)."""
            # Ensure metadata exists
            if not validated_attr.metadata or not validated_attr.metadata.extensions:
                return
            # For OUD, SOURCE == TARGET (OUD is RFC-compliant, no transformations)
            if validated_attr.syntax:
                validated_attr.metadata.extensions[
                    meta_keys.SCHEMA_TARGET_SYNTAX_OID
                ] = str(validated_attr.syntax)
            if validated_attr.name:
                validated_attr.metadata.extensions[
        ...
        def _add_oud_metadata(
            self,
            validated_attr: FlextLdifModels.SchemaAttribute,
            attr_definition: str,
        ) -> None:
            """Add OUD metadata to attribute for bidirectional conversion."""
            meta_keys = FlextLdifConstants.MetadataKeys

            # Add GENERIC metadata keys for 100% bidirectional conversion (NO *_OUD_* keys!)
            if not validated_attr.metadata:
                validated_attr.metadata = self.create_metadata(attr_definition.strip())

            if not validated_attr.metadata:
                return

            # Add source metadata fields
            self._add_source_metadata_fields(validated_attr, meta_keys, attr_definition)

            # Preserve SOURCE matching rules
            source_rules = self._build_source_rules(validated_attr)
            if source_rules:
                validated_attr.metadata.extensions[
                    meta_keys.SCHEMA_SOURCE_MATCHING_RULES
                ] = source_rules

            # Add target metadata fields
            self._add_target_metadata_fields(validated_attr, meta_keys, source_rules)
        ...
        def _parse_attribute(
            ...
            *,
            ...
            Args:
                ...
                _case_insensitive: Ignored - OUD uses strict RFC-compliant NAME matching
            ...
            try:
                ...
                hook_result = self._hook_post_parse_attribute(attr)

                if hook_result.is_failure:
                    return hook_result

                # Get validated attribute from hook
                validated_attr = hook_result.unwrap()

                # Add GENERIC metadata keys for 100% bidirectional conversion
                self._add_oud_metadata(validated_attr, attr_definition)

                return FlextResult[FlextLdifModels.SchemaAttribute].ok(validated_attr)

            except Exception as e:
                logger.exception(
                    "OUD attribute parsing failed",
                )
                return FlextResult[FlextLdifModels.SchemaAttribute].fail(
        ...
        def _parse_objectclass(
            ...
            try:
                ...
                logger.exception(
                    "OUD objectClass parsing failed",
                )
                return FlextResult[FlextLdifModels.SchemaObjectClass].fail(
        ...
        def _write_attribute(
            ...
            ✅ STRICT RULE: OUD Writer SEMPRE aplica transformações RFC → OUD LDIF
            Não importa de onde veio (OID, OUD, OpenLDAP, etc.)
            Se estamos escrevendo OUD LDIF, SEMPRE aplicamos conversões OUD!

            Args:
                attr_data: SchemaAttribute model (in RFC format)

            Returns:
                FlextResult with OUD-compatible attribute string

            """
            # ✅ SEMPRE aplica transformações OUD (EQUALITY→SUBSTR, etc.)
            # Não verifica source_server - writer SEMPRE escreve no formato do server!
            transformed_attr = self._transform_attribute_for_write(attr_data)

            # Call RFC base writer with OUD-transformed attribute
        ...
        def _write_objectclass(
            ...
            Args:
                oc_data: SchemaObjectClass model (in RFC format)

            Returns:
                FlextResult with OUD-compatible objectClass string

            """
            # ✅ SEMPRE aplica transformações OUD
            # Não verifica source_server - writer SEMPRE escreve no formato do server!
            transformed_oc = self._transform_objectclass_for_write(oc_data)

            # Call RFC base writer with OUD-transformed objectClass
    ...
    class Acl(FlextLdifServersRfc.Acl):
        ...
        def _get_oud_acl_attributes(cls) -> list[str]:
            """Get OUD native ACL attributes.

            Returns:
                List of OUD-specific ACL attributes (ds-privilege-name only).
                OUD uses RFC 4876 'aci' attribute as foundation (inherited from RFC).
        ...
        def can_handle_acl(self, acl_line: FlextLdifTypes.AclOrString) -> bool:
            ...
            # Must NOT contain "access to" (alternative ACL format), parentheses (ACI format), or equals (attribute format)
            return bool(
                normalized
                and not any(
        ...
        def _parse_aci_format(self, acl_line: str) -> FlextResult[FlextLdifModels.Acl]:
            ...
            try:
                ...
                if not validation_result.is_success:
                    error_msg = (
                        validation_result.error or "ACI format validation failed"
                    )
                ...
                logger.exception(
                    "Failed to parse OUD ACI format",
                )
                return FlextResult[FlextLdifModels.Acl].fail(
        ...
        def _build_oud_permissions(
            ...
            permissions_data = FlextLdifUtilities.ACL.build_permissions_dict(
                permissions_list, perm_map
            )

            # Handle "all" permission special case
            if FlextLdifUtilities.Schema.is_attribute_in_list(
                FlextLdifServersOud.Constants.PERMISSION_ALL, permissions_list
            ):
                permissions_data = dict.fromkeys(permissions_data, True)
        ...
        def _build_oud_subject(
            ...
            return FlextLdifUtilities.ACL.build_acl_subject(
                bind_rules_data, subject_type_map, special_values
            )
        ...
        def _create_acl_from_context(
            ...
            try:
                ...
                # Build permissions using helper (DRY refactoring)
                permissions_data = self._build_oud_permissions(permissions_list)

                # Build subject using helper (DRY refactoring)
                subject_type, subject_value = self._build_oud_subject(bind_rules_data)

                # Parse targetattr using helper (DRY refactoring)
                acl_line = context.get("original_acl_line", "")
                targetattr_str = cast("str", context["targetattr"])
                target_attributes, target_dn = self._parse_targetattr(targetattr_str)

                # Padronizar metadados GENÉRICOS para conversão bidirecional
                # Preservar tipo original do subject no metadata para que QUALQUER quirk possa escrever corretamente
                # Usar MetaKeys GENÉRICOS padronizados - servidores NÃO podem saber um do outro
                original_oud_subject_type = subject_type  # OUD original type

                metadata_extensions = {
                    **extensions,
                    # GENERIC metadata keys (for ANY LDAP server conversion)
                    FlextLdifConstants.MetadataKeys.ACL_ORIGINAL_FORMAT: cast(
                        "str",
                        acl_line,
                    ),
                    FlextLdifConstants.MetadataKeys.ACL_SOURCE_SERVER: "oud",  # OUD parsed this ACI
                    FlextLdifConstants.MetadataKeys.ACL_SOURCE_SUBJECT_TYPE: original_oud_subject_type,  # Original OUD type
                    FlextLdifConstants.MetadataKeys.ACL_TARGET_SUBJECT_TYPE: subject_type,  # Target type (may be normalized)
                }
        ...
        def _build_aci_target(self, acl_data: FlextLdifModels.Acl) -> str:
            ...
            """
            # Get target from model field or metadata
            ...
            # Use attributes list if available
            if target.attributes:
                target_attr_str = " || ".join(target.attributes)
                return f'(targetattr="{target_attr_str}")'

            # Use target_dn if no attributes list
            if target.target_dn and target.target_dn != "*":
                target_attr = target.target_dn
                return f'(targetattr="{target_attr}")'
        ...
        def _extract_acl_metadata(
            ...
            """Extract base_dn and source_subject_type from ACL metadata."""
            if not (
                acl_data.metadata
            ...
            base_dn = base_dn_value if isinstance(base_dn_value, str) else None

            source_subject_type_value = acl_data.metadata.extensions.get(
                MetaKeys.ACL_SOURCE_SUBJECT_TYPE,
            )
            source_subject_type = (
                cast("str | None", source_subject_type_value)
                if isinstance(source_subject_type_value, str)
                else None
            )

            return base_dn, source_subject_type

        def _determine_subject_type(
            self,
            acl_data: FlextLdifModels.Acl,
            source_subject_type: str | None,
        ) -> str:
            """Determine subject type for formatting."""
            subject_type_for_format = (
                acl_data.subject.subject_type if acl_data.subject else None
            )

            # If no subject parsed, try to use source_subject_type from metadata
            if subject_type_for_format is None and source_subject_type:
                subject_type_for_format = source_subject_type

            # If we still don't have subject info, default to self
            if not subject_type_for_format:
                return "self"

            # If subject_type is one of the attribute-based types, use it directly
            if subject_type_for_format in {"dn_attr", "guid_attr", "group_attr"}:
                return subject_type_for_format

            if subject_type_for_format == "bind_rules":
                # Check source_subject_type from metadata to determine original type
                if source_subject_type in {"dn_attr", "guid_attr", "group_attr"}:
                    return source_subject_type
                if source_subject_type == "group_dn":
                    return "group"
                if (
                    acl_data.subject
                    and acl_data.subject.subject_value
                    and (
                        "group=" in acl_data.subject.subject_value.lower()
                        or "groupdn" in acl_data.subject.subject_value.lower()
                    )
                ):
                    return "group"
        ...
        def _get_subject_value(
            self,
            acl_data: FlextLdifModels.Acl,
            subject_type_for_format: str,
        ) -> str:
            """Get subject value for formatting."""
            subject_value_for_format = (
                acl_data.subject.subject_value if acl_data.subject else None
            )

            # If no subject parsed, try to get from metadata
            if (
                not subject_value_for_format
                and acl_data.metadata
                and acl_data.metadata.extensions
            ):
                subject_value_raw = acl_data.metadata.extensions.get(
                    MetaKeys.ORIGINAL_SUBJECT_VALUE
                )
                subject_value_for_format = (
                    cast("str | None", subject_value_raw)
                    if subject_value_raw is not None
                    else None
                )

            # Default to self if no value
            if not subject_value_for_format and subject_type_for_format == "self":
                return FlextLdifServersOud.Constants.ACL_SELF_SUBJECT

            return subject_value_for_format or ""
        ...
        def _build_aci_subject(self, acl_data: FlextLdifModels.Acl) -> str:
            ...
            """
            # Extract metadata for subject formatting
            base_dn, source_subject_type = self._extract_acl_metadata(acl_data)

            # Determine subject type for formatting
            subject_type_for_format = self._determine_subject_type(
                acl_data,
                source_subject_type,
            )

            # Get subject value
            subject_value_str = self._get_subject_value(
                acl_data, subject_type_for_format
            )

            # Default to self if no subject type
            if not subject_type_for_format or subject_type_for_format == "self":
    ...
    class Entry(FlextLdifServersRfc.Entry):
        ...
        def can_handle(
            ...
            entry_attrs = (
                dict(attributes)
                if not FlextRuntime.is_dict_like(attributes)
                else attributes
            )
        ...
        def _process_attribute_value(
            ...
            if (
                ...
                valid_numbers = [
                    v for v in telephone_values if FlextRuntime.is_valid_phone(v)
                ]
                if valid_numbers:
            ...
            return [
                ...
                for val in (
                    [attr_values]
                    if not FlextRuntime.is_list_like(attr_values)
                    else attr_values
                )
            ]
        ...
        def _build_metadata_extensions(
            ...
            # Get original attributes from entry
            original_attrs = entry.attributes.attributes if entry.attributes else {}

            # Build GENERIC metadata for 100% bidirectional conversion (NO *_OUD_* keys!)
            metadata_extensions: dict[str, object] = {
                # CORE ENTRY METADATA (required for ALL servers)
                meta_keys.ENTRY_SOURCE_SERVER: "oud",  # OUD parsed this entry
                meta_keys.ENTRY_ORIGINAL_FORMAT: "OUD Entry (RFC-compliant)",
                # SOURCE values (original OUD format - BEFORE transformation)
                # For OUD, SOURCE == TARGET because OUD is RFC-compliant (no transformations)
                meta_keys.ENTRY_SOURCE_DN_CASE: dn_str,  # Original OUD DN
                meta_keys.ENTRY_SOURCE_ATTRIBUTES: list(original_attrs.keys()),
                meta_keys.ENTRY_SOURCE_OBJECTCLASSES: original_attrs.get(
                    "objectClass",
                    [],
                ),
                # TARGET values (RFC format - AFTER transformation)
                # For OUD: TARGET == SOURCE (OUD is already RFC-compliant)
                meta_keys.ENTRY_TARGET_DN_CASE: dn_str,  # Same as source
                meta_keys.ENTRY_TARGET_ATTRIBUTES: list(processed_attributes.keys()),
                meta_keys.ENTRY_TARGET_OBJECTCLASSES: processed_attributes.get(
                    "objectClass",
                    [],
                ),
                ...
                meta_keys.ENTRY_SOURCE_OPERATIONAL_ATTRS: [
                    ...
                    in {
                        a.lower()
                        for a in FlextLdifServersOud.Constants.OPERATIONAL_ATTRIBUTES
                    }
                ],
                # Attribute conversions tracking (empty for OUD - no conversions needed)
                meta_keys.CONVERTED_ATTRIBUTES: {
                    "attribute_name_conversions": {},  # OUD uses RFC attribute names
                    "boolean_conversions": {},  # OUD uses RFC boolean format (TRUE/FALSE)
                },
            }

            # Preserve DN spaces (OUD-specific quirk)
            ...
            # Preserve attribute order (OUD-specific quirk)
        ...
        def _normalize_and_process_attributes(
            self,
            entry: FlextLdifModels.Entry,
        ) -> dict[str, list[str]]:
            ...
            Args:
                entry: Base entry from parent parser

            Returns:
                Dictionary of normalized attribute names to processed values

            """
            attrs_dict = (
                entry.attributes.attributes if entry.attributes is not None else {}
            )
            final_attributes: dict[str, list[str]] = {}
            for attr_name, attr_values in attrs_dict.items():
                ...
                final_attributes[normalized_name] = self._process_attribute_value(
                    normalized_name, attr_values
                )
            return final_attributes

        def _populate_roundtrip_metadata(
            self,
            entry: FlextLdifModels.Entry,
            new_metadata: FlextLdifModels.QuirkMetadata,
            entry_dn: str,
        ) -> None:
            """Populate metadata fields for complete round-trip support.

            Args:
                entry: Original entry
                new_metadata: Metadata to populate
                entry_dn: Original DN string
            ...
            for attr_name in original_attrs:
                ...
                if normalized != attr_name:
                    new_metadata.original_attribute_case[normalized] = attr_name

            if "objectClass" in original_attrs:
                for oc in original_attrs["objectClass"]:
                    oc_lower = oc.lower() if isinstance(oc, str) else str(oc)
        ...
        def _analyze_oud_entry_differences(
            self,
            original_entry_dict: dict[str, object],
            final_attributes: dict[str, list[str]],
            original_dn: str,
            parsed_dn: str | None,
        ) -> tuple[dict[str, object], dict[str, dict[str, object]]]:
            """Analyze DN and attribute differences for OUD entries.

            Args:
                original_entry_dict: Original entry attributes
                final_attributes: Final processed attributes
                original_dn: Original DN string
                parsed_dn: Parsed DN value

            Returns:
                Tuple of (dn_differences, attribute_differences)
            ...
            for attr_name, attr_values in original_entry_dict.items():
                parsed_values = final_attributes.get(attr_name, [])

                # Build string representations
                original_str = f"{attr_name}: {', '.join(str(v) for v in (attr_values if FlextRuntime.is_list_like(attr_values) else [attr_values]))}"
                parsed_str = (
                    f"{attr_name}: {', '.join(str(v) for v in parsed_values)}"
                    if parsed_values
                    else None
                )
                ...
                attr_diff = FlextLdifUtilities.Metadata.analyze_minimal_differences(
                    ...
                    converted=parsed_str if parsed_str != original_str else None,
                    context="attribute",
                )
                attribute_differences[attr_name] = attr_diff

            return dn_differences, attribute_differences

        def _store_oud_minimal_differences(
            self,
            metadata: FlextLdifModels.QuirkMetadata,
            ...
            """Store minimal differences in OUD entry metadata.

            Args:
                metadata: QuirkMetadata to update (internal domain type)
            ...
            if dn_differences.get("has_differences"):
                FlextLdifUtilities.Metadata.track_minimal_differences_in_metadata(
                    ...
                    original=original_dn,
                    converted=parsed_dn
                    if parsed_dn and parsed_dn != original_dn
                    else None,
                    context="dn",
                    attribute_name="dn",
                )
        ...
        def _parse_entry(
            ...
            try:
                ...
                final_attributes = self._normalize_and_process_attributes(entry)

                combined_attributes = final_attributes.copy()
                ...
                metadata_extensions["original_entry"] = entry

                # Analyze differences using helper (DRY refactoring)
                original_dn = entry_dn
                parsed_dn = entry.dn.value if entry.dn else None
                dn_differences, attribute_differences = (
                    self._analyze_oud_entry_differences(
                        original_entry_dict, final_attributes, original_dn, parsed_dn
                    )
                )

                # Store minimal differences in metadata_extensions
                metadata_extensions["minimal_differences_dn"] = dn_differences
                metadata_extensions["minimal_differences_attributes"] = (
                    attribute_differences
                )
                metadata_extensions["original_dn_complete"] = original_dn
                metadata_extensions["original_attributes_complete"] = {
                    str(k): (
                        list(v)
                    ...
                    for k, v in original_entry_dict.items()
                }
                ...
                # ZERO DATA LOSS: Preserve original lines in metadata
                if not new_metadata.original_format_details:
                    new_metadata.original_format_details = {}
                new_metadata.original_format_details["original_dn_line"] = (
                    original_dn_line
                )
                new_metadata.original_format_details["original_attr_lines"] = (
                    original_attr_lines
                )
                new_metadata.original_format_details["minimal_differences_dn"] = (
                    dn_differences
                )
                new_metadata.original_format_details[
                ...
                # Track minimal differences using helper (DRY refactoring)
                self._store_oud_minimal_differences(
                ...
                self._populate_roundtrip_metadata(entry, new_metadata, entry_dn)

                logger.debug(
                    "OUD entry parsed with complete minimal differences analysis",
                    entry_dn=original_dn[:50] if original_dn else None,
                    # Before/After comparison
                    original_dn=original_dn,
                    parsed_dn=parsed_dn,
                    original_attributes_count=len(original_entry_dict),
                    parsed_attributes_count=len(final_attributes),
                    # What was preserved
                    minimal_differences_dn_captured=bool(
                        dn_differences.get("has_differences", False)
                    ),
                    minimal_differences_attributes_captured=len(attribute_differences)
                    > 0,
                )
        ...
        def _create_oid_dn_replacer(
            ...
            """Create alternative ACL format DN replacement function for re.sub.
            ...
            def replace_dn_oid(match: re.Match[str]) -> str:
                """Replace DN in alternative ACL format ACL string."""
                dn_value = match.group(2)
                # Clean DN: remove spaces after commas (server-specific quirk fix)
                cleaned_dn = FlextLdifUtilities.DN.clean_dn(dn_value)
                ...
                if (
                    base_dn
                    and cleaned_dn
                    and not FlextLdifUtilities.DN.is_under_base(cleaned_dn, base_dn)
                ):
                    dn_was_filtered[0] = True
                    return match.group(0)  # Keep original if filtered

                # Return normalized ACL bind rule (preserve original format)
        ...
        def _normalize_single_aci_value(
            ...
            # Then normalize alternative ACL format (by group="..." or by dn="...")
            normalized_aci = re.sub(
                r'by\s+(group|dn)="([^"]+)"',
                replace_dn_oid,
                normalized_aci,
        ...
        def _restore_original_dn(
            ...
            if not (original_dn and isinstance(original_dn, str) and entry_data.dn):
                return entry_data
            ...
            logger.debug(
                "Restored original OUD DN from metadata",
                original_dn=original_dn,
        ...
        def _restore_original_attributes(
            ...
            logger.debug(
                "Restored original OUD attributes from metadata",
                original_attrs_count=len(original_attrs),
                restored_attrs_count=len(restored_attrs),
            )
            return entry_data.model_copy(
                update={
        ...
        def _hook_pre_write_entry(
            ...
            This hook ensures that Entry data with RFC-valid syntax is properly
            formatted for OUD LDIF output. It implements bidirectional conversion:
            - If source_server == "oud": Restore original OUD format from SOURCE metadata
            - If source_server != "oud": Apply OUD transformations from RFC
            ...
            """
            # ✅ SEMPRE converte RFC+metadata → OUD (SEM verificar source_server!)
            # Entry model é SEMPRE RFC (aci:, ds-privilege-name:, etc.)
            # Aplicamos validações OUD (syntax, ACI macros, etc.)
            # Isso VALIDA que conversão funciona corretamente!
            return FlextLdifServersOud._hook_pre_write_entry_static(
                entry,
                self._validate_aci_macros,
                self._correct_rfc_syntax_in_attributes,
        ...
        def _finalize_and_parse_entry(
            ...
            if result.is_success:
                ...
                # Analyze DN differences (original vs parsed)
                original_dn = dn
                parsed_dn = entry.dn.value if entry.dn else None
                dn_differences = (
                    FlextLdifUtilities.Metadata.analyze_minimal_differences(
                        original=original_dn,
                        converted=parsed_dn
                        if parsed_dn and parsed_dn != original_dn
                        else None,
                ...
                # Analyze attribute differences for each attribute
                attribute_differences: dict[str, dict[str, object]] = {}
                parsed_attrs = entry.attributes.attributes if entry.attributes else {}

                for attr_name, attr_values in original_entry_dict.items():
                    parsed_values = parsed_attrs.get(attr_name, [])

                    # Build original string representation
                    original_attr_str = f"{attr_name}: {', '.join(str(v) for v in (attr_values if FlextRuntime.is_list_like(attr_values) else [attr_values]))}"
                    parsed_attr_str = (
                        f"{attr_name}: {', '.join(str(v) for v in parsed_values)}"
                        if parsed_values
                        else None
                    ...
                    attr_diff = FlextLdifUtilities.Metadata.analyze_minimal_differences(
                        original=original_attr_str,
                        converted=parsed_attr_str
                        if parsed_attr_str != original_attr_str
                        else None,
                        context="attribute",
                    )
                    attribute_differences[attr_name] = attr_diff
                ...
                entry.metadata.extensions["minimal_differences_dn"] = dn_differences
                entry.metadata.extensions["minimal_differences_attributes"] = (
                    attribute_differences
                )
                entry.metadata.extensions["original_dn_complete"] = original_dn
                entry.metadata.extensions["original_attributes_complete"] = {
                    str(k): (
                        list(v)
                        if isinstance(v, (list, tuple))
                        else [str(v)]
                ...
                logger.debug(
                    ...
                    parsed_dn=parsed_dn,
                    original_attributes_count=len(original_entry_dict),
                    parsed_attributes_count=len(parsed_attrs),
                    # What was preserved
                    minimal_differences_dn_captured=bool(
                        dn_differences.get("has_differences", False)
                    ),
