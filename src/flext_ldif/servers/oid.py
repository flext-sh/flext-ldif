    ...
    **Validation**: Verify protocol compliance with:
    ...
    class Schema(FlextLdifServersRfc.Schema):
        ...
        def __init__(
            ...
            self.acl = FlextLdifServersOid.Acl(server_type=server_type)
            ...
            try:
                ...
                if (
                    ...
                    if name_lower in auxiliary_without_sup:
                        schema_oc.sup = "top"
                        logger.debug(
                            f"Adding missing SUP top to AUXILIARY class {schema_oc.name}",
                        )

                # Fix kind mismatches with superior classes
                if schema_oc.sup and schema_oc.kind:
                    ...
                    sup_lower = (
                        str(schema_oc.sup).lower()
                        if isinstance(schema_oc.sup, str)
                        else ""
                    )
                    ...
                    if (
                        ...
                        logger.debug(
                            f"Changing {schema_oc.name} from AUXILIARY to STRUCTURAL "
                    ...
                    # If SUP is AUXILIARY but objectClass is STRUCTURAL, change to AUXILIARY
                        ...
                        logger.debug(
                            f"Changing {schema_oc.name} from STRUCTURAL to AUXILIARY "
                            f"to match superior class {schema_oc.sup}",
                        )
                ...
                if not schema_oc.metadata:
                    schema_oc.metadata = FlextLdifModels.QuirkMetadata.create_for_quirk(
                        quirk_type=FlextLdifConstants.ServerTypes.OID,
            ...
            try:
                ...
                if not sup_value and kind_value == FlextLdifConstants.Schema.AUXILIARY:
                    ...
                    if name_lower in auxiliary_without_sup:
                        sup_value = "top"
                        logger.debug(
                            "Adding missing SUP top to AUXILIARY class %s",
                            name_value,
                        )
                ...
                if sup_value and kind_value:
                    ...
                    sup_lower = (
                        str(sup_value).lower() if isinstance(sup_value, str) else ""
                    )
                    ...
                    if (
                        ...
                        logger.debug(
                            "Changing %s from AUXILIARY to STRUCTURAL to match superior class %s",
                            name_value,
                            sup_value,
                        )
                    ...
                    # If SUP is AUXILIARY but objectClass is STRUCTURAL, change to AUXILIARY
                        ...
                        logger.debug(
                            "Changing %s from STRUCTURAL to AUXILIARY to match superior class %s",
                            name_value,
                            sup_value,
                        )
        ...
        def write_attribute_to_rfc(
            ...
            try:
                ...
                if ";binary" in name:
                    ...
                    logger.debug("Removed ;binary suffix from attribute name: %s", name)
                # Replace underscore with hyphen (OUD doesn't allow _ in names)
                if "_" in name:
                    name = name.replace("_", "-")
                    logger.debug(
                        "Replaced underscore with hyphen in attribute name: %s",
                        name,
                    )
                ...
                if attr_data.equality:
                    ...
                    if equality in self.MATCHING_RULE_REPLACEMENTS:
                        ...
                        logger.debug(
                            "Replaced matching rule %s with %s",
                            original_eq,
                            equality,
                        )
                ...
                return FlextResult[str].fail(
                    f"Failed to write OID attribute to RFC: {e}",
                )
        ...
        def write_objectclass_to_rfc(
            ...
            try:
                ...
                return FlextResult[str].fail(
                    f"Failed to write OID objectClass to RFC: {e}",
                )
    ...
    class Acl(FlextLdifServersRfc.Acl):
        ...
        Example:
            quirk = FlextLdifServersOid.Acl(server_type="oracle_oid")
            if quirk.can_handle_acl(acl_line):
                result = quirk.parse_acl(acl_line)

        """

        # OVERRIDE: Oracle OID uses "orclaci" for ACL attribute names (not inherited)
        ...
        def convert_acl_from_rfc(
            ...
            try:
                ...
                oid_acl = acl_data.model_copy(
                    update={"server_type": FlextLdifConstants.ServerTypes.OID},
                )
    ...
    class Entry(FlextLdifServersRfc.Entry):
        ...
        Example:
            quirk = FlextLdifServersOid.Entry(server_type="oracle_oid")
        ...
        # --------------------------------------------------------------------- #
        # These methods override the base class with Oracle OID-specific logic:
        # - can_handle_entry(): Detects OID entries by DN/attributes
        # - process_entry(): Normalizes OID entries with metadata
        # - convert_entry_to_rfc(): Converts OID entries to RFC format
        ...
        def convert_entry_to_rfc(
            ...
            try:
                ...
                # NOTE: Filtering is NOT the responsibility of quirks.
                # All attribute filtering is handled by AlgarOudMigConstants
