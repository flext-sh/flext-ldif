...
class FlextLdifServersOudAcl(FlextLdifServersRfcAcl):
    ...
    **OUD Extensions** (Oracle Docs):
    ...
    4. **Special Subjects** (Oracle-specific):
       ...
       - ``userdn="ldap:///anyone"``: All authenticated users (including anonymous)
    ...
    **Multiple Permissions per ACI**::
    ...
    def can_handle(self, acl_line: t.Ldif.AclOrString) -> bool:
        ...
        """
        # Type narrowing: validate acl_line is str or m.Ldif.Acl
        if isinstance(acl_line, str):
            return self.can_handle_acl(acl_line)
        if isinstance(acl_line, m.Ldif.Acl):
            return self.can_handle_acl(acl_line)
        # Not a valid ACL type
        return False
    ...
    def _parse_acl(self, acl_line: str) -> FlextResult[m.Ldif.Acl]:
        ...
        **OUD Override** (this method):
        ...
        2. **ds-privilege-name format** (OUD administrative privileges)::
    ...
    def _parse_aci_format(self, acl_line: str) -> FlextResult[m.Ldif.Acl]:
        ...
        **RFC Override**: This method extends RFC behavior (RFC has no ACI parser).
        ...
        # Type narrowing: m.Ldif.Acl inherits from m.Ldif.Acl
    ...
    def _parse_ds_privilege_name(
        ...
        **OUD Implementation** (this method):
        - Parses OUD-specific administrative privilege names
        - Creates minimal ACL model with privilege stored in metadata
        - Used for OUD server configuration and administrative access control

        OUD ds-privilege-name Format
        ----------------------------

        Oracle OUD uses simple privilege names (not full ACI format) for
        administrative access control. These are typically found in the
        ...
        **Model Factory**:
        ...
        - Uses ``m.Ldif.QuirkMetadata()`` for server-specific metadata
        ...
        **RFC Override**: This is OUD-only (RFC has no ds-privilege-name concept).
        ...
        try:
            ...
            # Use StrEnum member for type-safe Literal compatibility
            ...
            acl_model = m.Ldif.Acl(
                ...
                server_type=server_type_oud,  # OUD server type
                ...
                metadata=m.Ldif.QuirkMetadata(
                    quirk_type=c.Ldif.ServerTypes.OUD,  # OUD quirk type from StrEnum
    ...
    def _should_use_raw_acl(self, acl_data: FlextLdifModelsDomains.Acl) -> bool:
        ...
        Args:
            acl_data: ACL model instance (base or derived type)
    ...
    def _build_aci_target(self, acl_data: FlextLdifModelsDomains.Acl) -> str:
        ...
        **Attribute Separator**: OUD uses ``||`` (double pipe) to separate
        ...
        if not target and acl_data.metadata:
            # Type narrowing: acl_data.metadata is m.Ldif.QuirkMetadata
    ...
    def _build_aci_subject(self, acl_data: FlextLdifModelsDomains.Acl) -> str:
        ...
        **OUD Implementation** (this method):
        ...
        ::
            ...
            userdn = "ldap:///self"
            userdn = "ldap:///cn=admin,dc=example,dc=com"
            groupdn = "ldap:///cn=admins,ou=groups,dc=example,dc=com"
            roledn = "ldap:///cn=dir-admin,dc=example,dc=com"
    ...
    def _write_acl(self, acl_data: FlextLdifModelsDomains.Acl) -> FlextResult[str]:
        ...
        **ACI Components Built**:
        ...
        5. **Bind rules** (from ``_build_aci_subject``):
           ...
           - ``userdn="ldap:///cn=admin,dc=example,dc=com"`` for user DN
           - ``groupdn="ldap:///cn=admins,ou=groups,dc=example,dc=com"`` for group
