...
class FlextLdifServersOudEntry(FlextLdifServersRfc.Entry):
    ...
    **Entry with Complex ACI** (multi-line)::
    ...
    def _comment_acl_attributes(
        ...
        acl_attribute_names: Sequence[str],
    ) -> m.Ldif.Entry:
    ...
    def _create_write_options_with_hidden_attrs(
        ...
        if isinstance(write_opts, FlextLdifModelsDomains.WriteOptions):
            write_opts_data: Mapping[str, t.NormalizedValue] = write_opts.model_dump()
            write_opts_data["hidden_attrs"] = list(hidden_attrs_set)
            return m.Ldif.WriteOptions.model_validate(write_opts_data)
        write_opts_dict: Mapping[str, t.NormalizedValue] = {
    ...
    def _normalize_acl_values(
        ...
        Returns:
            Normalized values as Sequence[str], str, or Acl model
    ...
    def apply_syntax_corrections(
        ...
        corrected_data: Mapping[
            str,
            t.Scalar | Sequence[str] | Mapping[str, str | Sequence[str]] | None,
        ],
        syntax_corrections: Sequence[str] | Mapping[str, str] | None,
    ) -> r[m.Ldif.Entry]:
        ...
        attrs_for_model: Mapping[str, Sequence[str]] = {}
    ...
    def correct_syntax_and_return_entry(
        ...
        corrected_data_typed: Mapping[
            str,
            t.Scalar | Sequence[str] | Mapping[str, str | Sequence[str]] | None,
        ] = dict(corrected_data)
        syntax_corrections_raw = corrected_data_typed.get("syntax_corrections")
        syntax_corrections_typed: Sequence[str] | Mapping[str, str] | None = None
        if isinstance(syntax_corrections_raw, list):
            ...
            syntax_corrections_dict: Mapping[str, str] = {}
    ...
    def extract_and_remove_acl_attributes(
        ...
        acl_attribute_names: Sequence[str],
    ) -> tuple[Mapping[str, Sequence[str]], Mapping[str, Sequence[str]], set[str]]:
        ...
        """
        new_attrs: Mapping[str, Sequence[str]] = dict(attributes_dict)
        commented_vals: Mapping[str, Sequence[str]] = {}
    ...
    def update_metadata_with_commented_acls(
        ...
        acl_attribute_names: Sequence[str],
        commented_acl_values: Mapping[str, Sequence[str]],
        hidden_attrs: set[str],
        entry_attributes_dict: Mapping[str, Sequence[str]],
        ...
        if commented_acl_values:
            converted_attrs_list: Sequence[str] = list(commented_acl_values.keys())
        ...
        update_dict_final: Mapping[str, t.NormalizedValue] = {
    ...
    def parse(self, value: str) -> r[Sequence[m.Ldif.Entry]]:
        ...
        processed_entries: Sequence[m.Ldif.Entry] = []
        ...
        return r[Sequence[m.Ldif.Entry]].ok(processed_entries)

    @override
    def parse_entry(
        self,
        entry_dn: str,
        entry_attrs: Mapping[str, Sequence[str]] | m.Ldif.Entry,
    ) -> r[m.Ldif.Entry]:
        ...
        **RFC Override**: Extends RFC (RFC creates Entry, OUD adds metadata).
        ...
        """
        entry_attrs_dict: Mapping[str, Sequence[str]] = {}
    ...
    def _add_acl_value_comments(
        ...
        original_attr: str,
        attr_name: str,
        acl_values: Sequence[str] | str | m.Ldif.Acl,
    ) -> None:
    ...
    def _add_oud_acl_comments(
        ...
        acl_comments_dict: Mapping[str, Sequence[str]] = {}
    ...
    def _add_transformation_comments(
        ...
        if (
            ...
            removed_attr_names: Sequence[str] = [
                str(attr_name)
        ...
        for src_key, dest_key in key_mapping.items():
            ...
            if value_raw is not None:
                if value_raw is None or u.is_primitive(value_raw):
                    ...
                    value_list: Sequence[t.Ldif.Scalar] = [
                    ...
                    value_dict_2: Mapping[str, t.Scalar] = {}
    ...
    def _extract_acl_metadata_from_dynamic(
        ...
        for src_key, dest_key in key_mapping.items():
            ...
            if u.is_primitive(value_raw):
                ...
                value_dict_1: Mapping[str, t.Scalar] = {}
                for k, v in value_raw.items():
                    key = str(k)
                    if u.is_primitive(v):
                        value_dict_1[key] = v
    ...
    def _finalize_and_parse_entry(
        ...
        if result.is_success:
            ...
            parsed_attrs: Mapping[str, Sequence[str]] = (
                dict(entry.attributes.attributes) if entry.attributes else {}
            )
            converted_attrs: Mapping[str, Sequence[str | bytes]] = {
                k: list(v) for k, v in parsed_attrs.items()
            }
            entry_attrs_for_diff: Mapping[
                str,
                t.Scalar | Sequence[str] | Mapping[str, str] | None,
            ] = {}
    ...
    def _find_aci_values(
        ...
        aci_values: Sequence[str] | str | None = None
        if original_attrs:
    ...
    def _normalize_aci_value_simple(
        ...
        """Normalize ACI value to Sequence[str] | str | None."""
    ...
    def _normalize_acl_dns(self, entry_data: m.Ldif.Entry) -> m.Ldif.Entry:
        ...
        **Base DN Filtering** (when configured):
        ...
        if not aci_values:
            return entry_data
        normalized_aci_values: Sequence[str] = []
    ...
    def _process_parsed_acl_extensions(
        ...
        for key, value in acl_extensions.items():
            ...
            if value is None or u.is_primitive(value):
                ...
                value_dict_inner: Mapping[str, t.Scalar] = {}
        ...
        if parse_result.is_success:
            ...
            if parsed_acl.metadata and parsed_acl.metadata.extensions:
                ...
                if core_u.is_type(
                    ...
                    acl_extensions_dict: Mapping[str, t.NormalizedValue] = {
                        str(
                            k,
                        ): FlextLdifModelsMetadata.DynamicMetadata.coerce_metadata_value(
                            v,
    ...
    def _restore_entry_from_metadata(self, entry_data: m.Ldif.Entry) -> m.Ldif.Entry:
        ...
        **2. Attribute Restoration** (if case mapping available):
        ...
        if (
            ...
            restored: Mapping[str, Sequence[str]] = {}
