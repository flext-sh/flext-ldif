    ...
    def resolve_objectclass_hierarchy(
        ...
        if object_class in schema.objectclasses:
            ...
            if isinstance(oc_def, dict):
                superior: Any = oc_def.get(
                    FlextLdifConstants.ServerDetection.SCHEMA_FIELD_SUPERIOR, []
                )
            else:
                # Assume it's a SchemaObjectClass model
                superior = getattr(
                    oc_def, FlextLdifConstants.ServerDetection.SCHEMA_FIELD_SUPERIOR, []
                )
    ...
    def get_all_required_attributes(
        ...
        for oc_name in object_classes:
            if oc_name in schema.objectclasses:
                ...
                if isinstance(oc_def, dict):
                    req_attrs: Any = oc_def.get(
                        FlextLdifConstants.ServerDetection.SCHEMA_FIELD_REQUIRED_ATTRIBUTES,
                        [],
                    )
                else:
                    # Assume it's a SchemaObjectClass model
    ...
    def get_all_optional_attributes(
        ...
        for oc_name in object_classes:
            if oc_name in schema.objectclasses:
                oc_def = schema.objectclasses[oc_name]
                if isinstance(oc_def, dict):
                    opt_attrs: Any = oc_def.get(
                        FlextLdifConstants.ServerDetection.SCHEMA_FIELD_OPTIONAL_ATTRIBUTES,
                        [],
                    )
                else:
                    # Assume it's a SchemaObjectClass model
    ...
    def validate_objectclass_combination(
        ...
        for oc_name in object_classes:
            if oc_name in schema.objectclasses:
                oc_def = schema.objectclasses[oc_name]
                if isinstance(oc_def, dict):
                    is_structural = oc_def.get(
                        FlextLdifConstants.ServerDetection.SCHEMA_FIELD_STRUCTURAL,
                        False,
                    )
                else:
                    # Assume it's a SchemaObjectClass model
                    is_structural = getattr(
                        oc_def,
                        FlextLdifConstants.ServerDetection.SCHEMA_FIELD_STRUCTURAL,
                        False,
                    )
