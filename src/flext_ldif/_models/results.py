...
class _StatisticsSummary(m.FrozenModel):
...
class _MigrationSummary(m.FrozenModel):
    statistics: _StatisticsSummary | None = Field(
        default=None, description="Aggregated statistics summary for the migration"
    )
...
class _Statistics(m.Statistics):
    ...
    events: MutableSequence[
        ...
        default_factory=_events_list_factory,
    ...
    @computed_field
    def summary(self) -> _StatisticsSummary:
    ...
    def merge(
        ...
        updates |= {
            "processing_duration": self.processing_duration + other.processing_duration,
    ...
    def to_summary(self) -> _StatisticsSummary:
        fields = {name: getattr(self, name) for name in _StatisticsSummary.model_fields}
        return _StatisticsSummary(**fields)
...
class _MigrationPipelineResult(m.FrozenModel):
    ...
    @computed_field
    def migration_summary(self) -> _MigrationSummary:
...
class _ClientStatus(m.Value):
    ...
    services: Annotated[
        MutableSequence[str], Field(description="Available service names")
    ]
    config: Annotated[
...
class _ValidationResult(m.FrozenModel):
    is_valid: Annotated[
        bool, Field(description="Whether all entries passed validation")
    ]
    ...
    errors: Annotated[
        MutableSequence[str], Field(description="Validation error messages")
    ]
...
class _EntryAnalysisResult(m.FrozenModel):
...
class _ServerDetectionResult(m.FrozenModel):
...
class _StatisticsResult(m.StrictModel):
    ...
    rejection_rate: Annotated[
        float, Field(description="Percentage of entries rejected")
    ]
...
class _EntriesStatistics(m.Value):
...
class _DictAccessibleValue(m.Value):
...
class _SchemaServiceStatus(_DictAccessibleValue):
    ...
    rfc_compliance: Annotated[
        str, Field(description="RFC compliance level of the schema service")
    ]
    operations: Annotated[
        MutableSequence[str], Field(description="Supported schema operations")
    ]


class _SyntaxServiceStatus(_DictAccessibleValue):
    service: Annotated[str, Field(description="Syntax service name")]
    status: Annotated[str, Field(description="Current service operational status")]
    rfc_compliance: Annotated[
        str, Field(description="RFC compliance level of the syntax service")
    ]
    total_syntaxes: Annotated[
        int, Field(description="Total number of registered syntaxes")
    ]
    common_syntaxes: Annotated[
        int, Field(description="Number of commonly used syntaxes")
    ]


class _StatisticsServiceStatus(_DictAccessibleValue):
    service: Annotated[str, Field(description="Statistics service name")]
    status: Annotated[str, Field(description="Current service operational status")]
    capabilities: Annotated[
        MutableSequence[str], Field(description="Supported statistics capabilities")
    ]
    version: Annotated[str, Field(description="Service version identifier")]


class _ValidationServiceStatus(_DictAccessibleValue):
    service: Annotated[str, Field(description="Validation service name")]
    status: Annotated[str, Field(description="Current service operational status")]
    rfc_compliance: Annotated[
        str, Field(description="RFC compliance level of the validation service")
    ]
    validation_types: Annotated[
        MutableSequence[str], Field(description="Supported validation types")
    ]


class _ValidationBatchResult(m.StrictModel):
    results: Annotated[
...
class _ParseResponse(m.Value):
    # model_config: frozen + validate_default fully inherited from m.Value
    entries: MutableSequence[FlextLdifModelsDomains.Entry] = Field(
        ...,
        description="Parsed LDIF entries",
    )
    statistics: _Statistics = Field(..., description="Parsing statistics")
    detected_server_type: c.Ldif.ServerTypeLiteral | None = Field(
        default=None, description="LDAP server type detected during parsing"
    )


class _AclResponse(m.Value):
    # model_config: frozen + validate_default fully inherited from m.Value
    acls: MutableSequence[FlextLdifModelsDomains.Acl] = Field(
        ...,
        description="Extracted ACL models",
    )
    statistics: _Statistics = Field(..., description="ACL extraction statistics")


class _AclEvaluationResult(m.Value):
    # model_config: frozen + validate_default fully inherited from m.Value
    ...
    message: str = Field(
        default="", description="Human-readable evaluation result message"
    )


class _WriteResponse(m.Value):
    # model_config: frozen + validate_default fully inherited from m.Value
    content: str | None = Field(
        default=None, description="Serialized LDIF content string"
    )
    statistics: _Statistics = Field(..., description="Write operation statistics")


class FlextLdifModelsResults:
    @staticmethod
    def _statistics_factory() -> _Statistics:
        return _Statistics()

    StatisticsSummary = _StatisticsSummary
    MigrationSummary = _MigrationSummary
    Statistics = _Statistics
    MigrationPipelineResult = _MigrationPipelineResult
    ClientStatus = _ClientStatus
    ValidationResult = _ValidationResult
    EntryAnalysisResult = _EntryAnalysisResult
    ServerDetectionResult = _ServerDetectionResult
    StatisticsResult = _StatisticsResult
    EntriesStatistics = _EntriesStatistics
    DictAccessibleValue = _DictAccessibleValue
    SchemaServiceStatus = _SchemaServiceStatus
    SyntaxServiceStatus = _SyntaxServiceStatus
    StatisticsServiceStatus = _StatisticsServiceStatus
    ValidationServiceStatus = _ValidationServiceStatus
    ValidationBatchResult = _ValidationBatchResult
    ParseResponse = _ParseResponse
    AclResponse = _AclResponse
    AclEvaluationResult = _AclEvaluationResult
