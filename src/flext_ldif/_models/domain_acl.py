...
class _AclPermissions(m.ArbitraryTypesModel):
    ...
    def get_rfc_compliant_permissions(
        ...
        return {
            key: value for key, value in perms_dict.items() if key in rfc_compliant_keys
        }


class _AclTarget(m.ArbitraryTypesModel):
...
class _AclSubject(m.ArbitraryTypesModel):
...
class _Acl(FlextLdifModelsBases.AclElement):
...
class _AclWriteMetadata(m.FrozenModel):
...
class FlextLdifModelsDomainAcl:
    ...
    AclPermissions = _AclPermissions
    AclTarget = _AclTarget
    AclSubject = _AclSubject
    Acl = _Acl
    AclWriteMetadata = _AclWriteMetadata
