...
from flext_ldif._models.domain_metadata import _QuirkMetadata
from flext_ldif.constants import FlextLdifConstants as c


class _SchemaAttribute(FlextLdifModelsBases.SchemaElement):
...
class _Syntax(FlextLdifModelsBases.SchemaElement):
    ...
    def resolve_syntax_oid(
        ...
        try:
            ...
            type_category = (
                c.Ldif.NAME_TO_TYPE_CATEGORY.get(name, "string") if name else "string"
            )
            metadata = (
...
class _SchemaObjectClass(FlextLdifModelsBases.SchemaElement):
...
class FlextLdifModelsDomainSchema:
    ...
    Syntax = _Syntax
    SchemaObjectClass = _SchemaObjectClass
