...
class FlextLdifRfcLdifParser(FlextService[dict[str, object]]):
    ...
    Example:
        ...
        params = {"file_path": "oid.ldif", "source_server": FlextLdifConstants.ServerTypes.OID}
        parser = FlextLdifRfcLdifParser(params=params, quirk_registry=registry)
        result = parser.execute()
    ...
    def execute(self) -> FlextResult[dict[str, object]]:
        ...
        try:
            ...
            content_raw = self._params.get(
                FlextLdifConstants.ServerDetection.CONTENT_PARAMETER
            )
            if content_raw:
                ...
                parse_changes_raw = self._params.get(
                    FlextLdifConstants.ServerDetection.PARSE_CHANGES_PARAMETER,
                    FlextLdifConstants.ServerDetection.DEFAULT_PARSE_CHANGES,
                )
                ...
                if self.logger is not None:
                    self.logger.info(
                        FlextLdifConstants.ServerDetection.MSG_PARSING_LDIF_CONTENT,
                        extra={
                            "content_length": len(content),
                            FlextLdifConstants.ServerDetection.PARSE_CHANGES_PARAMETER: content_parse_changes,
                        },
                    )
                ...
                if self.logger is not None:
                    self.logger.info(
                        FlextLdifConstants.ServerDetection.MSG_LDIF_CONTENT_PARSED,
                        extra={
            ...
            if not file_path_str:
                return FlextResult[dict[str, object]].fail(
                    f"Either '{FlextLdifConstants.Parameters.FILE_PATH}' or '{FlextLdifConstants.ServerDetection.CONTENT_PARAMETER}' parameter is required"
                )
            ...
            if self.logger is not None:
                self.logger.info(
                    f"{FlextLdifConstants.ServerDetection.MSG_PARSING_LDIF_FILE}: {file_path}",
                    extra={
                        FlextLdifConstants.Parameters.FILE_PATH: str(file_path),
                        FlextLdifConstants.ServerDetection.PARSE_CHANGES_PARAMETER: file_parse_changes,
                        FlextLdifConstants.DictKeys.ENCODING: encoding,
                    },
                )
            ...
            if file_parse_result.is_failure:
                return FlextResult[dict[str, object]].fail(file_parse_result.error)
            ...
            if self.logger is not None:
                self.logger.info(
                    FlextLdifConstants.ServerDetection.MSG_LDIF_PARSED_SUCCESS,
                    extra={
                        "total_entries": len(entries),
                    },
                )
            ...
            error_msg = (
                f"{FlextLdifConstants.ServerDetection.MSG_FAILED_EXECUTE_PARSER}: {e}"
            )
            if self.logger is not None:
        ...
        try:
            ...
            if content is not None:
                ...
                return FlextResult[list[FlextLdifModels.Entry]].fail(
                    FlextLdifConstants.ServerDetection.MSG_EITHER_CONTENT_OR_PATH_REQUIRED
                )
