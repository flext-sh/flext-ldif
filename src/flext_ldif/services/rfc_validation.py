...
class FlextLdifValidation(FlextLdifServiceBase[m.Ldif.ValidationServiceStatus]):
    ...
    attribute_names: Annotated[MutableSequence[str], Field(default_factory=list)] = Field(default_factory=list)
    objectclass_names: Annotated[MutableSequence[str], Field(default_factory=list)] = Field(default_factory=list)
    ...
    def validate_attribute_names(
        ...
        try:
            validated_names: MutableMapping[str, bool] = {}
            for name in names:
            ...
            validated_mapping: MutableMapping[str, bool] = validated_names
            return r[MutableMapping[str, bool]].ok(validated_mapping)
        except (ValueError, TypeError, AttributeError) as e:
            return r[MutableMapping[str, bool]].fail(
    ...
    def with_attribute_names(self, names: MutableSequence[str]) -> Self:
    ...
    def with_objectclass_names(self, names: MutableSequence[str]) -> Self:
