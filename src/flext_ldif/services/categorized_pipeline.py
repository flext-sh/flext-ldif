...
class FlextLdifCategorizedMigrationPipeline(
    ...
    - Uses FlextLdifEntrys for entry-level transformations
    ...
    def __init__(
        ...
        *,
        ...
        source_schema_quirk: FlextLdifServersBase.Schema | None = None,
        target_schema_quirk: FlextLdifServersBase.Schema | None = None,
        ...
        self._output_files: dict[str, str] = output_files or {
            ...
            FlextLdifConstants.Categories.ACL: "acl.ldif",
            FlextLdifConstants.Categories.REJECTED: "rejected.ldif",
        }
    ...
    def execute(self) -> FlextResult[FlextLdifModels.PipelineExecutionResult]:
        ...
        categorized[FlextLdifConstants.Categories.ACL] = extracted_acl_entries

        # Log ACL extraction results
        self.logger.info(
            f"ACL extraction phase: Populated '{FlextLdifConstants.Categories.ACL}' category with "
        ...
        # Step 5: Convert dictionaries to proper Entry objects and write LDIF files
        entries_by_category: dict[str, list[FlextLdifModels.Entry]] = {}
        written_counts: dict[str, int] = {}

        # Initialize writer service
        writer_config = FlextLdifConfig(
            quirks_server_type=self._target_server,
        ...
        writer = FlextLdifWriterService(
            ...
            quirk_registry=cast(
                "FlextLdifRegistry",
                getattr(self.container, "resolve", lambda _: None)("quirk_registry"),
            ),
        ...
        for category, entries in transformed_categorized.items():
            ...
            for i, entry_dict in enumerate(entries):
                ...
                try:
                    ...
                    if objectclass_list:
                        converted_attributes["objectClass"] = objectclass_list
            ...
            # Write this category to LDIF file using unified writer
            ...
            write_result = writer.write_to_file(entry_objects, output_path)
            if write_result.is_failure:
                return FlextResult[FlextLdifModels.PipelineExecutionResult].fail(
                    f"Failed to write {category}: {write_result.error}",
                )

            result_dict = write_result.unwrap()
            written_counts[category] = cast(
                "int",
                result_dict.get(
                    FlextLdifConstants.DictKeys.ENTRIES_WRITTEN,
                    len(entry_objects),
                ),
            )
        ...
        pipeline_stats = FlextLdifModels.PipelineStatistics(
            ...
            acl_entries=len(
                transformed_categorized.get(FlextLdifConstants.Categories.ACL, []),
            ),
            rejected_entries=len(
                transformed_categorized.get(FlextLdifConstants.Categories.REJECTED, []),
            ),
    ...
    class _LdifFileParsingChain:
        ...
        def parse_ldif_file(
            ...
            _quirk_registry: Registry for RFC parser quirks (unused)
            ...
            """
            try:
                # Use unified FlextLdifParserService for parsing
                parser = FlextLdifParserService()
                parse_result = parser.parse(ldif_file)
                ...
                if acl_count_models > 0:
                    logger.debug(
                        "DEBUG POINT 0: Entry MODELS with ACL attributes",
                        extra={
                            "file_name": ldif_file.name,
                            "acl_count": acl_count_models,
                            "total_entries": len(entries_raw),
                        },
                    )
                else:
                    logger.warning(
                        "DEBUG POINT 0: NO Entry MODELS with ACL attributes",
                        extra={
                            "file_name": ldif_file.name,
                            "checked_models": len(entries_raw),
                        },
                    )
                ...
                for entry_model in entries_raw:
                    ...
                    for (
                        ...
                        if hasattr(attr_values, "values"):
                            values_list = attr_values.values
                        else:
                            values_list = attr_values
                        if attr_name.lower() == FlextLdifConstants.DictKeys.OBJECTCLASS:
                            # Add to objectClass list for Entry model
                            entry_dict[FlextLdifConstants.DictKeys.OBJECTCLASS] = (
                                values_list
                            )
                            # CRITICAL FIX: Also add to attrs_dict so objectClass is written to LDIF
                            # The 'if' (not 'elif') ensures objectClass is preserved in output
                ...
                if acl_count > 0:
                    logger.debug(
                        "DEBUG POINT 1: Entries with ACL attributes after parsing",
                        extra={
                            "file_name": ldif_file.name,
                            "acl_count": acl_count,
    ...
    def _parse_entries(self) -> FlextResult[list[dict[str, object]]]:
        ...
        try:
            ...
            parsed_file_results, file_failures = FlextResult.batch_process(
                ldif_files,
                parse_ldif_file_processor,
            )

            # Log failures if any (non-fatal, continue with successful files)
            if file_failures:
                self.logger.debug(
                    "LDIF file parsing failures",
                    extra={"failed_file_count": len(file_failures)},
                )
    ...
    def categorize_entry(self, entry: dict[str, object]) -> tuple[str, str | None]:
        ...
        """
        # Convert dict to Entry object at method boundary
        dn_str = str(entry.get(FlextLdifConstants.DictKeys.DN, ""))

        # Extract attributes dict - may be nested or merged with top-level keys
        nested_attrs_raw = entry.get(FlextLdifConstants.DictKeys.ATTRIBUTES, {})
        nested_attrs: dict[str, list[str] | str] = (
            nested_attrs_raw if isinstance(nested_attrs_raw, dict) else {}
        )

        # Also gather top-level attributes (keys that aren't "dn" or "attributes")
        # Filter and type-cast to ensure only valid LDIF attribute types
        top_level_attrs: dict[str, list[str] | str] = {
            k: v
        ...
        attrs_dict: dict[str, list[str] | str] = {**nested_attrs, **top_level_attrs}

        # CRITICAL FIX: Normalize objectClass attribute name (RFC 4512 requires case-insensitive)
        # The LDIF file may have "objectclass" (lowercase), but Entry.create expects "objectClass"
        # Build normalized attrs with proper objectClass key
        normalized_attrs: dict[str, list[str] | str] = {}
        object_class_values: list[str] = []

        for attr_name, attr_value in attrs_dict.items():
            # Collect objectClass values regardless of case
            if attr_name.lower() == "objectclass":
                if isinstance(attr_value, list):
                    object_class_values.extend([str(v) for v in attr_value])
                else:
                    object_class_values.append(str(attr_value))
            else:
                normalized_attrs[attr_name] = attr_value

        # Add objectClass back with proper name if found
        if object_class_values:
            normalized_attrs["objectClass"] = object_class_values

        attrs_dict = normalized_attrs

        entry_result = FlextLdifModels.Entry.create(dn=dn_str, attributes=attrs_dict)
        if not entry_result.is_success:
            return (FlextLdifConstants.Categories.REJECTED, str(entry_result.error))

        entry_obj = entry_result.value
    ...
    def _categorize_entries(
        ...
        try:
            ...
            categorization_results, categorization_failures = FlextResult.batch_process(
                entries,
                categorize_entry_processor,
            )

            # Log failures if any (non-fatal, continue processing)
            if categorization_failures:
                self.logger.debug(
                    "Entry categorization failures",
                    extra={"failed_entry_count": len(categorization_failures)},
                )
            ...
            for result_item in categorization_results:
                ...
                if isinstance(processed_entry, dict):
                    # Apply base_dn filter for non-schema, non-ACL categories
                    # Schema entries (00) and ACL entries (04) are never filtered by base_dn
                    # Reason: Schema applies globally; ACL entries define permissions that may apply
                    # to entries anywhere in the directory, not just under base_dn
                    ...
                    if (
                        ...
                        categorized[FlextLdifConstants.Categories.REJECTED].append(
                            processed_entry,
                        )
                        if isinstance(dn_value, str):
                            rejection_reasons_map[dn_value] = base_dn_rejection
                        self.logger.debug(
                            "Filtered out entry not under base_dn",
                            extra={"entry_dn": dn_value},
                        )
                        continue
            ...
            rejected_entries = categorized.get(
                FlextLdifConstants.Categories.REJECTED,
                [],
            )
            if rejected_entries and rejection_reasons_map:
                ...
                injected_entries, injection_failures = FlextResult.batch_process(
                    rejected_entries,
                    inject_rejection_reason_processor,
                )

                # Log failures if any (non-fatal)
                if injection_failures:
                    self.logger.debug(
                        "Rejection reason injection failures",
                        extra={"failed_entry_count": len(injection_failures)},
                    )

                # Replace rejected entries with injected versions
                categorized[FlextLdifConstants.Categories.REJECTED] = injected_entries

            # DEBUG: Log ACL category details after categorization
            acl_count = len(categorized.get(FlextLdifConstants.Categories.ACL, []))
            if acl_count > 0:
                self.logger.debug(
                    "DEBUG POINT 3: ACL category after categorization",
                    extra={"acl_entry_count": acl_count},
                )
                first_acl = categorized[FlextLdifConstants.Categories.ACL][0]
                dn = first_acl.get(FlextLdifConstants.DictKeys.DN, "unknown")
                attrs = first_acl.get(FlextLdifConstants.DictKeys.ATTRIBUTES, {})
                if isinstance(attrs, dict):
                    self.logger.debug(
                        "DEBUG: First ACL entry",
                        extra={
                            "entry_dn": dn,
                            "attribute_keys": list(attrs.keys())[:5],
                        },
                    )
            else:
                self.logger.warning(
                    "DEBUG POINT 3: ACL category is EMPTY after categorization",
                    extra={"expected_minimum": 2302},
                )
    ...
    class _AclTransformationChain:
        ...
        def transform_acl_entry(
            entry: dict[str, object],
            parser_acl_quirk: FlextLdifServersBase.Acl | None,
            writer_acl_quirk: FlextLdifServersBase.Acl | None,
        ) -> FlextResult[dict[str, object]]:
    ...
    class _EntryCategorizationChain:
        ...
        def categorize_and_track(
            ...
            """
            try:
                # Type narrow pipeline to access categorize_entry
                if not hasattr(pipeline, "categorize_entry"):
                    return FlextResult[dict[str, object]].fail(
                        "Pipeline missing categorize_entry method",
                    )
                ...
                categorize_func = pipeline.categorize_entry
    ...
    def _extract_acl_entries_final_phase(
        ...
        - NO base_dn filtering (ACLs are system-wide, not entry-scoped)
        ...
        ACL entries are system-level security configurations that control access
        across the entire directory. Unlike data entries (users, groups), ACLs
        should NOT be filtered by base_dn since they apply globally.
        ...
        try:
            ...
            for entry in all_entries:
                ...
                # Check if entry has any ACL attributes with actual values
                # CRITICAL FIX: Must check not just that the attribute NAME exists,
                # but that it has actual non-empty values. Entries with empty ACL
                # attributes (null, [], empty list) should NOT be categorized as ACL.
                has_acl = False
                for attr_name in attributes:
                    ...
                    # Check if attribute has ANY non-empty value in ANY supported format
                    # Must be comprehensive to handle all formats from LDIF parser
                    if attr_value is None or attr_value in ("", []):
                        # Explicitly empty - skip
                        continue

                    if isinstance(attr_value, dict):
                        # Check for all dict-based formats (Pydantic or custom)
                        # Try 'values' key first (most common)
                        values_list: object = attr_value.get("values")
                        if isinstance(values_list, list) and values_list:
                            has_acl = True
                            break

                        # Try 'single_value' key (Pydantic model)
                        single_val: object = attr_value.get("single_value")
                        if single_val is not None and str(single_val).strip():
                            has_acl = True
                            break

                        # Try direct iteration - dict might be values themselves
                        # Some formats store attribute data directly in dict
                        if attr_value:  # Non-empty dict
                            has_acl = True
                            break
                    elif isinstance(attr_value, list):
                        # List format - could be list of strings or list of dicts
                        ...
                        # Any other non-empty object (handle objects from models)
                        has_acl = True
                        break
                ...
                # NOTE: ACL entries are system-level security configurations
                # They should be migrated regardless of base_dn setting, as they control
                # access across the entire directory (not just specific subtrees).
                # Unlike data entries (users, groups, hierarchy), ACLs are not scoped
                # to a single base_dn and must be preserved during migration.
                #
                # Previous implementation filtered ACL entries by base_dn, causing all
                # ACL data to be silently dropped. This fix removes that filtering.
                ...
                # Build ACL entry with proper LDIF structure: {DN: ..., ATTRIBUTES: {...}}
                # Extract ACL definition strings from the wrapped DEFINITION dicts
                acl_definition_strings: list[str] = []
                for acl_item in acl_items:
                    if (
                        isinstance(acl_item, dict)
                        and FlextLdifConstants.DictKeys.DEFINITION in acl_item
                    ):
                        definition: object = acl_item[
                            FlextLdifConstants.DictKeys.DEFINITION
                        ]
                        if isinstance(definition, str):
                            acl_definition_strings.append(definition)

                # Build entry with ATTRIBUTES structure for transformation chain compatibility
                # Transformation chain expects {DN: ..., ATTRIBUTES: {orclaci: [strings]}} format
                # ACL strings will be transformed orclaci→aci by quirks system
                acl_entry: dict[str, object] = {
                    ...
                    FlextLdifConstants.DictKeys.ATTRIBUTES: {
                        "orclaci": acl_definition_strings,
                    },
                }

                # Step 3: Apply parser quirk (OID → RFC)
                # Get parser's ACL quirk if available (nested Schema.acl)
                parser_acl_quirk = None
                if self._parser_quirk is not None and hasattr(
                    self._parser_quirk, "Schema"
                ):
                    schema_cls = getattr(self._parser_quirk, "Schema", None)
                    if schema_cls is not None:
                        # Create Schema instance to get acl quirk
                        # Schema requires server_type parameter for initialization
                        try:
                            schema_instance = schema_cls(
                                server_type=self._source_server
                            )
                            if hasattr(schema_instance, "acl"):
                                parser_acl_quirk = schema_instance.acl
                        except (TypeError, AttributeError) as e:
                            self.logger.debug(
                                f"Failed to instantiate parser Schema: {e}",
                            )

                # Step 4: Apply writer quirk (RFC → OUD: aci)
                # Get writer's ACL quirk if available (nested Schema.acl)
                writer_acl_quirk = None
                if self._writer_quirk is not None and hasattr(
                    self._writer_quirk, "Schema"
                ):
                    schema_cls = getattr(self._writer_quirk, "Schema", None)
                    if schema_cls is not None:
                        # Create Schema instance to get acl quirk
                        # Schema requires server_type parameter for initialization
                        try:
                            ...
                            if hasattr(schema_instance, "acl"):
                                writer_acl_quirk = schema_instance.acl
                        except (TypeError, AttributeError) as e:
                            self.logger.debug(
                                f"Failed to instantiate writer Schema: {e}",
                            )
    ...
    def _transform_categories(
        ...
        try:
            ...
            parser_acl_quirk: FlextLdifServersBase.Acl | None = None
            writer_acl_quirk: FlextLdifServersBase.Acl | None = None

            # Try to get ACL quirk from parser (source server)
            if self._parser_quirk is not None and hasattr(self._parser_quirk, "Acl"):
                acl_quirk_cls = getattr(self._parser_quirk, "Acl", None)
                if acl_quirk_cls is not None:
                    parser_acl_quirk = acl_quirk_cls()

            # Try to get ACL quirk from writer (target server)
            if self._writer_quirk is not None and hasattr(self._writer_quirk, "Acl"):
                acl_quirk_cls = getattr(self._writer_quirk, "Acl", None)
                if acl_quirk_cls is not None:
                    writer_acl_quirk = acl_quirk_cls()

            # Transform ACL entries (category ONLY) using batch processing
            ...
            acl_entries = categorized.get(FlextLdifConstants.Categories.ACL, [])
            if acl_entries:
                # Define processor function for batch_process composition
                ...
                categorized[FlextLdifConstants.Categories.ACL] = transformed_acl

            # Step 1.5: Apply Entry conversion (OID→RFC normalization for ALL entries)
            # Convert boolean attributes "0"/"1" → "TRUE"/"FALSE" per RFC 4517
            parser_entry_quirk = None
            if self._parser_quirk is not None and hasattr(self._parser_quirk, "Entry"):
                entry_quirk_cls = getattr(self._parser_quirk, "Entry", None)
            ...
            try:
                ...
                self.logger.debug(
                    "DN reference normalization skipped",
