...
class FlextLdifSyntax(FlextLdifServiceBase[m.Ldif.SyntaxServiceStatus]):
    ...
    @classmethod
    def _build_validator_map(cls) -> MutableMapping[str, Callable[[str], r[bool]]]:
    ...
    @d.track_operation()
    def execute(self) -> r[m.Ldif.SyntaxServiceStatus]:
    ...
    def lookup_name(self, name: str) -> r[str]:
        ...
        try:
            oid_raw = u.get(self._name_to_oid, name, default="")
    ...
    def lookup_oid(self, oid: str) -> r[str]:
        ...
        try:
            name_raw = u.get(self._oid_to_name, oid, default="")
    ...
    def resolve_syntax(
        ...
        try:
            ...
            return r[m.Ldif.Syntax].fail(f"Failed to create syntax: {oid} - {e}")
        if name:
            syntax.name = name
        if desc:
            syntax.desc = desc
