...
class FlextLdifRfcLdifWriter(FlextService[dict[str, object]]):
    ...
    def execute(self) -> FlextResult[dict[str, object]]:
        ...
        try:
            ...
            append_mode = self._params.get(FlextLdifConstants.DictKeys.APPEND, False)
            ...
            if not entries and not schema and not acls:
                return FlextResult[dict[str, object]].fail(
                    FlextLdifConstants.ServerDetection.MSG_AT_LEAST_ONE_REQUIRED
                )
            ...
            if write_to_file:
                ...
                with cast(
                    "TextIO",
                    output_file.open(
                        mode, encoding=FlextLdifConstants.Encoding.DEFAULT_ENCODING
                    ),
                ) as f:
                ...
                if self.logger is not None:
                    self.logger.info(
                        f"{FlextLdifConstants.ServerDetection.MSG_LDIF_FILE_WRITTEN}: {output_file}",
                        extra={
            ...
            if self.logger is not None:
                self.logger.info(
                    FlextLdifConstants.ServerDetection.MSG_LDIF_CONTENT_GENERATED,
                    extra={
            ...
            return FlextResult[dict[str, object]].ok({
                FlextLdifConstants.DictKeys.CONTENT: ldif_content,
                FlextLdifConstants.DictKeys.ENTRIES_WRITTEN: total_entries,
                FlextLdifConstants.DictKeys.LINES_WRITTEN: total_lines,
            })
            ...
            return FlextResult[dict[str, object]].fail(
                f"{FlextLdifConstants.ServerDetection.ERROR_LDIF_WRITE_FAILED}: {e}"
            )
        ...
        try:
            ...
            for entry in entries:
                if isinstance(entry, dict):
                    ...
                    entry_dn = str(entry.get(FlextLdifConstants.DictKeys.DN, ""))
                    ...
                    if isinstance(attributes, dict):
                        for attr_name, attr_values in attributes.items():
                            ...
                            # ACL attribute names are now centralized in constants
                            is_acl_attribute = (
                                attr_name.lower()
                ...
                # Handle Entry objects
                    ...
                    msg = f"{FlextLdifConstants.ServerDetection.ERROR_UNSUPPORTED_ENTRY_TYPE}: {type(entry)}"
            ...
            with output_file.open("w", encoding="utf-8") as f:
                ...
                for idx, entry in enumerate(entries):
                    if isinstance(entry, dict):
                        ...
                        normalized_dn = self._normalize_dn(entry_dn)
                        dn_line = (
                            f"{FlextLdifConstants.Format.DN_PREFIX} {normalized_dn}"
                        )
                        f.write(dn_line + "\n")
    ...
    def _write_schema_entries(
        ...
        try:
            ...
            attributes_raw: Any = schema.get(FlextLdifConstants.DictKeys.ATTRIBUTES, {})
            attributes: dict[str, object] = (
                attributes_raw if isinstance(attributes_raw, dict) else {}
            )

            objectclasses_raw: Any = schema.get(
    ...
    def _write_entries(
        ...
        try:
            ...
            for entry in entries:
                ...
                attributes_normalized: dict[str, object]
                ...
                if isinstance(entry, FlextLdifModels.Entry):
                    ...
                    if (
                        entry_objectclass
                        and FlextLdifConstants.DictKeys.OBJECTCLASS
                        not in attributes_normalized
                    ):
                        oc_values = (
                            entry_objectclass
    ...
    def _extract_acl_definitions(self, raw_acl: list[object] | object) -> list[str]:
