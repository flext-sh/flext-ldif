...
class FlextLdifQuirksManager(FlextService[dict[str, object]]):
    ...
    def detect_server_type(
        ...
        for entry in entries:
            ...
            if "top" in object_classes and entry.dn.value.lower().startswith("cn=schema"):
    ...
    def get_server_quirks(self, server_type: str | None = None) -> FlextResult[dict[str, object]]:
    ...
    def get_acl_attribute_name(self, server_type: str | None = None) -> FlextResult[str]:
        """Get ACL attribute name for server type."""
        quirks_result = self.get_server_quirks(server_type)
        if quirks_result.is_failure:
            return FlextResult[str].fail(quirks_result.error or "Failed to get server quirks")

        acl_attr = quirks_result.value.get("acl_attribute", "aci")
    ...
    def get_acl_format(self, server_type: str | None = None) -> FlextResult[str]:
        ...
        acl_format = quirks_result.value.get("acl_format", "generic")
