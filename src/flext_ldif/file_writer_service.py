...
class FlextLdifFileWriterService:
    ...
    def write_category_file(
        ...
        if not entries:
            return FlextResult[int].ok(
                FlextLdifConstants.ServerDetection.DEFAULT_ENTRY_COUNT
            )
    ...
    def create_target_schema_entry(
        ...
        transformed_attributetypes = self._transform_schema_via_rfc(
            all_attributetypes, FlextLdifConstants.DataTypes.ATTRIBUTE
        )
        transformed_objectclasses = self._transform_schema_via_rfc(
            all_objectclasses, FlextLdifConstants.DataTypes.OBJECTCLASS
        )
    ...
    def _comment_blocked_entries_in_file(self, ldif_file: str) -> None:
        ...
        try:
            ...
            with Path(ldif_file).open(
                "w", encoding=FlextLdifConstants.Encoding.DEFAULT_ENCODING
            ) as f:
