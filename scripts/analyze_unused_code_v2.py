...
- Code must be used in flext-ldif, flext-ldap, or algar-oud-mig
...
ALGAR_OUD_MIG_ROOT = FLEXT_ROOT / "algar-oud-mig"
...
ALGAR_OUD_MIG_SRC = (
    ALGAR_OUD_MIG_ROOT / "src" / "algar_oud_mig"
    if ALGAR_OUD_MIG_ROOT.exists()
    else None
)

# All source directories
ALL_SRC_DIRS = [
    d for d in [FLEXT_LDIF_SRC, FLEXT_LDAP_SRC, ALGAR_OUD_MIG_SRC] if d and d.exists()
]
...
ALGAR_OUD_MIG_TESTS = (
    ALGAR_OUD_MIG_ROOT / "tests" if ALGAR_OUD_MIG_ROOT.exists() else None
)

ALL_TEST_DIRS = [
    d
    for d in [FLEXT_LDIF_TESTS, FLEXT_LDAP_TESTS, ALGAR_OUD_MIG_TESTS]
    if d and d.exists()
]
...
def main() -> None:
    ...
    print("Checking usage in: flext-ldif, flext-ldap, algar-oud-mig")
