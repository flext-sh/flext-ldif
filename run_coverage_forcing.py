...
services_tests: list[Callable[[], Any]] = [
    ...
    (lambda: services.analytics.analyze_entries([test_entry]) if services.analytics else None),
    (lambda: services.analytics.get_objectclass_distribution([test_entry]) if services.analytics else None),
    (lambda: services.analytics.get_dn_depth_analysis([test_entry]) if services.analytics else None),
    (lambda: services.analytics.analyze_patterns([test_entry]) if services.analytics else None),
    ...
    (lambda: services.parser.parse_content("dn: cn=test,dc=com\\nattr: value") if services.parser else None),
    (lambda: services.parser.parse_ldif_file("nonexistent.ldif") if services.parser else None),
    (lambda: services.parser.validate_ldif_syntax("dn: test\\n") if services.parser else None),
    ...
    (lambda: services.validator.validate_entries([test_entry]) if services.validator else None),
    (lambda: services.validator.validate_entry(test_entry) if services.validator else None),
    (lambda: services.validator.validate_entry_structure(test_entry) if services.validator else None),
    (lambda: services.validator.validate_dn_format("cn=test,dc=com") if services.validator else None),
    ...
    (lambda: services.writer.write_entries_to_string([test_entry]) if services.writer else None),
    ...
    (lambda: services.writer.unparse([test_entry]) if services.writer else None),
    ...
    (lambda: services.transformer.transform_entries([], lambda x: x) if services.transformer else None),
    (lambda: services.transformer.transform_entries([test_entry], lambda x: x) if services.transformer else None),
    (lambda: services.transformer.normalize_dns([test_entry]) if services.transformer else None),
    ...
    (lambda: services.repository.find_entry_by_dn([test_entry], "cn=test,dc=com") if services.repository else None),
    (lambda: services.repository.filter_entries_by_attribute([test_entry], "cn", "test") if services.repository else None),
    (lambda: services.repository.filter_entries_by_objectclass([test_entry], "person") if services.repository else None),
    (lambda: services.repository.filter_entries_by_object_class([test_entry], "person") if services.repository else None),
    (lambda: services.repository.get_statistics([test_entry]) if services.repository else None),
