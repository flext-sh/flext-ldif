...
services_tests = [
    ...
    (lambda: FlextLDIFServices.AnalyticsService(entries=[test_entry], config=config).execute()),

    # Parser variations
    (lambda: FlextLDIFServices.ParserService(content="", config=config).parse_ldif_content("")),
    (lambda: FlextLDIFServices.ParserService(content="", config=config).parse_ldif_content("dn: cn=test,dc=com\\nattr: value")),
    (lambda: FlextLDIFServices.ParserService(content="", config=config).parse_entries("")),

    # Validator variations
    (lambda: FlextLDIFServices.ValidatorService(config=config).validate_entries([])),
    (lambda: FlextLDIFServices.ValidatorService(config=config).validate_entries([test_entry])),
    (lambda: FlextLDIFServices.ValidatorService(config=config).validate_ldif_entries([])),
    (lambda: FlextLDIFServices.ValidatorService(config=config).validate_ldif_entries([test_entry])),
    ...
    (lambda: FlextLDIFServices.WriterService(config=config).format_entry_for_display(test_entry)),

    # Transformer variations
    (lambda: FlextLDIFServices.TransformerService(config=config).transform_entries([])),
    (lambda: FlextLDIFServices.TransformerService(config=config).transform_entries([test_entry])),
    (lambda: FlextLDIFServices.TransformerService(config=config).normalize_entries([])),
    (lambda: FlextLDIFServices.TransformerService(config=config).normalize_entries([test_entry])),

    # Repository variations
    (lambda: FlextLDIFServices.RepositoryService(entries=[], config=config).execute()),
    (lambda: FlextLDIFServices.RepositoryService(entries=[test_entry], config=config).execute()),
]
